...
<ThemeAwareImage
  ...
  alt="Endpoints page — connection type, environment, and project for each endpoint"
/>
...
| `experiment_parameters` | Resolved [experiment parameters](/docs/experiments) for the current test run. Access individual values with dot notation: `{{ experiment_parameters.model }}`, `{{ experiment_parameters.temperature }}`. Only populated when the test run is associated with an experiment; empty dict otherwise. See [Using Experiment Parameters](#using-experiment-parameters) below. |
| `params` | Deprecated alias for `experiment_parameters`. Existing templates using `{{ params.model }}` still work but will log a deprecation warning. |
...
| `version_info` | The version of your application behind this endpoint, as a JSON object (prompt version, model, parameters). Recorded on the test run and shown in its Configuration tab, the test runs list, and trace span details. See [Version Information](/docs/endpoints/management#version-information). |
...
<CodeBlock filename="request-mapping-with-params.json" language="json">
  ...
  "messages": [
    {
      "role": "system",
      "content": "{{ experiment_parameters.system_prompt }}"
    },
    {
      "role": "user",
      "content": "{{ input }}"
  ...
  "max_tokens": {{ experiment_parameters.max_tokens }}
}`}
</CodeBlock>
...
When no experiment is associated with a test run, `experiment_parameters` is an empty object and unresolved references render as empty strings. You can use Jinja2 defaults to handle this gracefully:

<CodeBlock filename="request-mapping-with-defaults.json" language="json">
  {`{
  "model": "{{ experiment_parameters.model | default('gpt-4') }}",
  "temperature": {{ experiment_parameters.temperature | default(0.7) }}
}`}
</CodeBlock>
