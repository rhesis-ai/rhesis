...
- `version_info` — the version of the system behind the endpoint, as a JSON object. Recorded on the run and shown in its Configuration tab, the test runs list, and trace span details. Takes precedence over the version configured on the endpoint. See [Version Information](/docs/endpoints/management#version-information)
