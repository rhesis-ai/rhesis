...
## TestRuns.stats() / TestResults.stats() (removed)

`TestRuns.stats()`, `TestResults.stats()`, and `TestRun.stats()` raise `NotImplementedError`.
The backend endpoints they called (`/test_runs/stats`, `/test_results/stats`) were removed;
use `Insights` directly instead, as shown above.
