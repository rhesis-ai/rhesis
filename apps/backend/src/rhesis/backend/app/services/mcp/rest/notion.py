...
class NotionSource:
    ...
    async def _fetch_page(
        ...
        if include_children:
            for child in child_pages:
                try:
                    sub_docs = await self._fetch_page(
                        client, child["id"], include_children=True
                    )
    ...
    async def _request_blocks(
        ...
        response = await client.get(
            f"{_NOTION_API}/blocks/{block_id}/children", params=params
        )
        ...
        if response.status_code == 401:
            raise ValueError(
                "Invalid Notion token. Please check your NOTION_TOKEN credentials."
            )
