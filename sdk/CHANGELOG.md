...
### Added
-   Introduced `OWASPSynthesizer` for OWASP LLM Top 10 red-teaming capabilities.

### Changed
-   The SDK now natively supports and is used for multi-turn conversations, improving conversational flow and state management.
-   Refactored the internal connector execution model, removing local backend execution and introducing `EndpointContext`. This may affect how connectors are configured or deployed.
-   The `services/mcp` package has been replaced by `agents/mcp` for improved architecture and consistency.

### Fixed
-   Resolved an issue where `system_prompt` was incorrectly stripped in certain scenarios.
-   Improved observability for the Architect agent, providing coherent multi-turn traces and a better conversation view.
-   Addressed a bug where agent batch execution could block the event loop, improving responsiveness and performance.
-   Corrected metric evaluation failures that occurred during single-turn tests.
...
- Enhanced telemetry exporter with a retry mechanism and exponential backoff for improved reliability.

### Fixed
- Corrected Rhesis Architect to prevent the inclusion of metric links in its output.
- Resolved issues with Garak probe-coupled detectors, improving overall pipeline reliability.
- Fixed LiteLLM proxy strict JSON schema handling for structured output.
- Addressed an issue with model connection endpoint passthrough.
- Patched vulnerable dependencies identified by `pip-audit`.
