...
class TestGetLanguageModel:
    """Test the get_language_model function with various configurations."""

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_minimal_defaults(self, mock_rhesis_class):
        """Test get_language_model() with no parameters - uses all defaults."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        result = get_language_model()

        # Should use default provider and language model
        mock_rhesis_class.assert_called_once_with(
            model_name=DEFAULT_LANGUAGE_MODELS[DEFAULT_PROVIDER], api_key=None
        )
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_provider_only(self, mock_rhesis_class):
        """Test get_language_model("rhesis") - uses default model for provider."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        result = get_language_model("rhesis")

        mock_rhesis_class.assert_called_once_with(
            model_name=DEFAULT_LANGUAGE_MODELS["rhesis"], api_key=None
        )
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_provider_and_model(self, mock_rhesis_class):
        """Test get_language_model("rhesis", "custom-model") - specific provider and model."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        result = get_language_model("rhesis", "custom-model")

        mock_rhesis_class.assert_called_once_with(model_name="custom-model", api_key=None)
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_with_api_key(self, mock_rhesis_class):
        """Test get_language_model with API key."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        result = get_language_model("rhesis", "test-model", "test-api-key")

        mock_rhesis_class.assert_called_once_with(model_name="test-model", api_key="test-api-key")
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_shorthand_provider_model(self, mock_rhesis_class):
        """Test get_language_model("rhesis/custom-model") - shorthand notation."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance
    ...
    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_with_config_object(self, mock_rhesis_class):
        """Test get_language_model(config=ModelConfig(...))."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        config = LanguageModelConfig(provider="rhesis", model_name="config-model", api_key="config-key")

        result = get_language_model(config=config)

        mock_rhesis_class.assert_called_once_with(model_name="config-model", api_key="config-key")
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_config_with_kwargs_override(self, mock_rhesis_class):
        ...
        result = get_language_model(config=config, model_name="override-model")
    ...
    def test_get_language_model_gemini(self, mock_gemini_class):
        """Test get_language_model with gemini provider."""
        mock_instance = Mock(spec=BaseLLM)
        mock_gemini_class.return_value = mock_instance

        result = get_language_model("gemini", "gemini-model")

        mock_gemini_class.assert_called_once_with(model_name="gemini-model", api_key=None)
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.gemini.GeminiLLM")
    def test_get_language_model_gemini_with_default(self, mock_gemini_class):
        """Test get_language_model with gemini provider using default model."""
        mock_instance = Mock(spec=BaseLLM)
        mock_gemini_class.return_value = mock_instance

        result = get_language_model("gemini")

        mock_gemini_class.assert_called_once_with(model_name=DEFAULT_LANGUAGE_MODELS["gemini"], api_key=None)
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.openrouter.OpenRouterLLM")
    def test_get_language_model_openrouter(self, mock_openrouter_class):
        """Test get_language_model with openrouter provider."""
        mock_instance = Mock(spec=BaseLLM)
        mock_openrouter_class.return_value = mock_instance

        result = get_language_model("openrouter", "anthropic/claude-3.5-sonnet")
    ...
    @patch("rhesis.sdk.models.providers.openrouter.OpenRouterLLM")
    def test_get_language_model_openrouter_with_default(self, mock_openrouter_class):
        """Test get_language_model with openrouter provider using default model."""
        mock_instance = Mock(spec=BaseLLM)
        mock_openrouter_class.return_value = mock_instance

        result = get_language_model("openrouter")

        mock_openrouter_class.assert_called_once_with(
            model_name=DEFAULT_LANGUAGE_MODELS["openrouter"], api_key=None
        )
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.openrouter.OpenRouterLLM")
    def test_get_language_model_openrouter_shorthand(self, mock_openrouter_class):
        """Test get_language_model with openrouter shorthand syntax."""
        mock_instance = Mock(spec=BaseLLM)
        mock_openrouter_class.return_value = mock_instance

        result = get_language_model("openrouter/meta-llama/llama-3.1-8b-instruct")
    ...
    def test_get_language_model_vertex_ai(self, mock_vertex_ai_class):
        """Test get_language_model with vertex_ai provider."""
        mock_instance = Mock(spec=BaseLLM)
        mock_vertex_ai_class.return_value = mock_instance

        result = get_language_model("vertex_ai", "gemini-2.5-flash")

        mock_vertex_ai_class.assert_called_once_with(model_name="gemini-2.5-flash")
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.vertex_ai.VertexAILLM")
    def test_get_language_model_vertex_ai_with_default(self, mock_vertex_ai_class):
        """Test get_language_model with vertex_ai provider using default model."""
        mock_instance = Mock(spec=BaseLLM)
        mock_vertex_ai_class.return_value = mock_instance

        result = get_language_model("vertex_ai")

        mock_vertex_ai_class.assert_called_once_with(model_name=DEFAULT_LANGUAGE_MODELS["vertex_ai"])
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.vertex_ai.VertexAILLM")
    def test_get_language_model_vertex_ai_shorthand(self, mock_vertex_ai_class):
        """Test get_language_model with vertex_ai shorthand syntax."""
        mock_instance = Mock(spec=BaseLLM)
        mock_vertex_ai_class.return_value = mock_instance

        result = get_language_model("vertex_ai/gemini-2.0-flash")

        mock_vertex_ai_class.assert_called_once_with(model_name="gemini-2.0-flash")
        assert result == mock_instance

    def test_get_language_model_unsupported_provider(self):
        """Test get_language_model with unsupported provider raises ValueError."""
        with pytest.raises(ValueError, match="Provider unsupported-provider not supported"):
            get_language_model("unsupported-provider")

    def test_get_language_model_unsupported_provider_from_config(self):
        """Test get_language_model with unsupported provider from config raises ValueError."""
        config = LanguageModelConfig(provider="unsupported-provider")

        with pytest.raises(ValueError, match="Provider unsupported-provider not supported"):
            get_language_model(config=config)

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_priority_order(self, mock_rhesis_class):
    ...
    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_none_values_use_defaults(self, mock_rhesis_class):
        """Test that None values fall back to defaults."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        result = get_language_model(None, None, None)
    ...
    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_empty_string_provider(self, mock_rhesis_class):
        """Test get_language_model with empty string provider - should use default."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        result = get_language_model("", "test-model")

        mock_rhesis_class.assert_called_once_with(model_name="test-model", api_key=None)
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_extra_params_passed_through(self, mock_rhesis_class):
        ...
        result = get_language_model("rhesis", "test-model", extra_params={"temperature": 0.5})
...
class TestModelFactoryIntegration:
    ...
    def test_rhesis_integration(self, mock_rhesis_class):
        ...
        models = [
            get_language_model(),  # defaults
            get_language_model("rhesis"),  # provider only
            get_language_model("rhesis", "custom-model"),  # provider + model
            get_language_model("rhesis/custom-model"),  # shorthand
            get_language_model(config= LanguageModelConfig(provider="rhesis", model_name="custom-model")),  # config
        ]
    ...
    def test_gemini_integration(self, mock_gemini_class):
        ...
        models = [
            get_language_model("gemini"),  # provider only
            get_language_model("gemini", "custom-model"),  # provider + model
            get_language_model("gemini/custom-model"),  # shorthand
            get_language_model(config= LanguageModelConfig(provider="gemini", model_name="custom-model")),  # config
        ]
    ...
    def test_openrouter_integration(self, mock_openrouter_class):
        ...
        models = [
            get_language_model("openrouter"),  # provider only
            get_language_model("openrouter", "anthropic/claude-3.5-sonnet"),  # provider + model
            get_language_model("openrouter/anthropic/claude-3.5-sonnet"),  # shorthand
            get_language_model(
    ...
    def test_vertex_ai_integration(self, mock_vertex_ai_class):
        ...
        models = [
            get_language_model("vertex_ai"),  # provider only
            get_language_model("vertex_ai", "custom-model"),  # provider + model
            get_language_model("vertex_ai/custom-model"),  # shorthand
            get_language_model(
...
class TestModelFactoryEdgeCases:
    ...
    def test_get_language_model_with_invalid_config_type(self):
        """Test get_language_model with invalid config type."""
        with pytest.raises(AttributeError):
            get_language_model(config="not-a-config-object")

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_with_very_long_model_name(self, mock_rhesis_class):
        """Test get_language_model with very long model name."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        long_name = "a" * 1000
        result = get_language_model("rhesis", long_name)

        mock_rhesis_class.assert_called_once_with(model_name=long_name, api_key=None)
        assert result == mock_instance

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_with_special_characters(self, mock_rhesis_class):
        """Test get_language_model with special characters in model name."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        special_name = "model-with-special-chars!@#$%^&*()"
        result = get_language_model("rhesis", special_name)

        mock_rhesis_class.assert_called_once_with(model_name=special_name, api_key=None)
        assert result == mock_instance

    def test_get_language_model_provider_case_sensitivity(self):
        ...
        with pytest.raises(ValueError, match="Provider Rhesis not supported"):
            get_language_model("Rhesis")

    @patch("rhesis.sdk.models.providers.native.RhesisLLM")
    def test_get_language_model_api_key_none_vs_empty_string(self, mock_rhesis_class):
        """Test that None and empty string API keys are handled the same."""
        mock_instance = Mock(spec=BaseLLM)
        mock_rhesis_class.return_value = mock_instance

        # Both should result in api_key=None being passed
        get_language_model("rhesis", "test-model", None)
        get_language_model("rhesis", "test-model", "")
...
class TestGetAvailableLanguageModels:
    ...
    def test_get_available_language_models_openai(self, mock_get_language_models):
        """Test getting available language models for OpenAI."""
        mock_get_language_models.return_value = ["gpt-4", "gpt-3.5-turbo", "gpt-4-turbo"]

        models = get_available_language_models("openai")

        assert models == ["gpt-4", "gpt-3.5-turbo", "gpt-4-turbo"]
        mock_get_language_models.assert_called_once()

    @patch("rhesis.sdk.models.providers.gemini.GeminiLLM.get_available_models")
    def test_get_available_language_models_gemini(self, mock_get_language_models):
        """Test getting available language models for Gemini."""
        mock_get_language_models.return_value = ["gemini-2.0-flash", "gemini-2.5-flash"]
...
class TestGetAvailableEmbeddingModels:
    ...
    def test_get_available_embedding_models_openai(self, mock_get_language_models):
        """Test getting available embedding models for OpenAI."""
        mock_get_language_models.return_value = [
    ...
    def test_get_available_embedding_models_gemini(self, mock_get_language_models):
        """Test getting available embedding models for Gemini."""
        mock_get_language_models.return_value = ["text-embedding-004", "gemini-embedding-001"]
    ...
    def test_get_available_embedding_models_vertex_ai(self, mock_get_language_models):
        """Test getting available embedding models for Vertex AI."""
        mock_get_language_models.return_value = ["text-embedding-005", "text-embedding-004"]
