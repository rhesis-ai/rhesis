...
class TestSingleTurnRunnerWithProvider:
    ...
    async def test_defaults_to_single_turn_output(self):
        ...
        with (
            ...
            patch(
                "rhesis.backend.jobs.execution.executors.runners.SingleTurnOutput",
            ) as mock_st_class,
        ):
    ...
    async def test_evaluates_metrics_with_injected_provider(self):
        ...
        with (
            ...
            patch(
                "rhesis.backend.jobs.execution.executors.runners.normalize_context_to_list",
                return_value=[],
            ),
            patch(
                "rhesis.backend.jobs.execution.executors.runners.MetricEvaluator",
            ),
            patch(
                "rhesis.backend.jobs.execution.executors.runners.evaluate_single_turn_metrics",
...
class TestMultiTurnRunnerWithProvider:
    ...
    async def test_defaults_to_multi_turn_output(self):
        ...
        with (
            patch(
                "rhesis.backend.jobs.execution.executors.runners.MultiTurnOutput",
            ) as mock_mt_class,
            patch(
                "rhesis.backend.jobs.execution.executors.runners.evaluate_multi_turn_metrics",
                return_value={},
