"""sync_rhesis_conversational_metrics_to_orgs

This migration intelligently syncs newly added Rhesis conversational metrics from
initial_data.json to all existing organizations. It's fully idempotent and can be
run multiple times - only missing metrics will be added.

New metrics added (from initial_data.json):
1. Conversation Completeness (Task Success) - Reliability
2. Contextual Coherence - Reliability
3. Conversational Relevance - Reliability
4. Knowledge Retention (Memory Consistency) - Reliability
5. Role Adherence (Instruction Compliance) - Compliance

These metrics use the ConversationalJudge class with custom prompts for multi-turn
