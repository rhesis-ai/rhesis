...
built-in [file format filters](/docs/endpoints/single-turn#file-format-filters)
...
The `to_anthropic` filter converts images to `\{"type": "image", "source": \{"type": "base64", ...\}\}` blocks and PDFs to `\{"type": "document", ...\}` blocks.
...
The `to_openai` filter converts each file to `\{"type": "image_url", "image_url": \{"url": "data:...;base64,..."\}\}`.
...
The `to_gemini` filter converts each file to `\{"inline_data": \{"mime_type": "...", "data": "..."\}\}`.
...
See [Multi-Turn Conversations](/docs/endpoints/multi-turn-conversations)
for details.
...
[Multi-Turn Conversations](/docs/endpoints/multi-turn-conversations) for
details.
...
<CodeBlock filename="test-context-nested.json" language="json">
  {`{
  "message": "{{ input }}",
  "tracking": {
    "test_id": "{{ test_id }}",
...
<CodeBlock filename="test-context-flat.json" language="json">
  {`{
  "prompt": "{{ input }}",
  "test_id": "{{ test_id }}",
  "test_run_id": "{{ test_run_id }}",
...
<CodeBlock filename="test-context-metadata.json" language="json">
  {`{
  "message": "{{ input }}",
  "metadata": {
    "source": "rhesis",
...
<Callout type="default">
  **Next Steps**
  ...
  - Explore [SDK Endpoints](/docs/endpoints/sdk-endpoints) for code-first
    endpoint definition
</Callout>
