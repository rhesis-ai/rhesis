...
The application uses a native authentication system with a pluggable provider architecture:
...
- Mitigates leaks from DB dumps/backups and many SQL-injection reads
- ciphertext useless without `DB_ENCRYPTION_KEY`
- Aligns with common compliance expectations when paired with other controls
...
- Passwords are hashed with bcrypt (via passlib) and stored securely in the database
