...
class TestQueryBuilder:
    ...
    def test_with_related_warns_when_load_count_exceeds_threshold(
    ...
    def test_with_related_passes_options_straight_to_query(
        self, test_db: Session, authenticated_user_id, test_org_id
    ):
        """with_related is a pass-through onto query.options() -- picking
        joinedload/selectinload and scoping columns is include()'s job, not
        with_related's; see test_query_builder_load.py for that behavior."""
        from rhesis.backend.app.utils.query_utils import include
        ...
        stage = MagicMock()
        original_query.options.return_value = stage

        result = query_builder.with_related(
            include(models.Test.prompt), include(models.Test.children)
        )
