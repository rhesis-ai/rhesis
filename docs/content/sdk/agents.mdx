...
Use `get_rhesis_tools()` to connect an agent to the Rhesis backend MCP endpoint. It reads `RHESIS_BASE_URL` and `RHESIS_API_KEY` from SDK configuration unless you pass explicit values. Note that the SDK uses `RHESIS_API_KEY` — this is distinct from `RHESIS_API_TOKEN`, which is used for direct REST authentication.
