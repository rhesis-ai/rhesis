...
<Callout type="info">
  The **PRD skill** (`rhesis-prd`) builds your evaluation baseline from requirements. The main [**Agent Skill**](/docs/agent-skill) (`rhesis`) explores endpoints, runs tests, and analyzes results. Install both for the full workflow.
</Callout>

## Prerequisites

Same as the [Agent Skill](/docs/agent-skill):
...
- The Rhesis MCP server connected in your agent (see [Agent Skill — Install in Cursor](/docs/agent-skill#install-in-cursor))
...
Install the main Agent Skill first, then add the PRD skill:
...
Connect the MCP server as described in [Agent Skill](/docs/agent-skill#install-in-cursor).
...
## Agent Skill vs. PRD skill

| | `rhesis` (Agent Skill) | `rhesis-prd` (PRD skill) |
| --- | --- | --- |
