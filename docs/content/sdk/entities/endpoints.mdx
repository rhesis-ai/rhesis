...
| `timeout_seconds` | `int \| None` | Per-endpoint invocation timeout (1 -- 3600). When unset, REST endpoints default to 30s and SDK endpoints to 120s. |
...
endpoint = Endpoint(
    ...
    timeout_seconds=60,  # optional, overrides the default
)
endpoint.push()`}
</CodeBlock>
...
`endpoint.get_annotations()` returns every human verdict left on results and traces produced against this endpoint. See [Annotations](/sdk/entities/annotations).
