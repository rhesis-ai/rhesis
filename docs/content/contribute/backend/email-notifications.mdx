...
{`from rhesis.backend.jobs.base import BaseJob, with_tenant_context, email_notification
...
@app.task(base=BaseJob, name="your.user.facing.task", bind=True)
@with_tenant_context
...
- `generate_and_save_test_set` — `TASK_COMPLETION`
- `import_garak_probes`, `sync_garak_test_set` — `TASK_COMPLETION`
...
the summary email path is the same as for sequential runs. See `jobs/execution/shared.py` and
`jobs/execution/results.py`.
...
On success or permanent failure, `BaseJob` (in its `on_success` / `on_failure` hooks) loads the
