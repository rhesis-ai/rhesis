...
def create_test_organization_and_user(
    ...
    try:
        ...
        print(f"🔧 Setting up tenant context for organization {organization.id}")
...
def cleanup_test_data(db: Session, organization_id: Optional[str] = None) -> None:
    ...
    try:
        if organization_id:
            ...
            test_orgs = db.query(models.Organization).filter(
                models.Organization.name.like("%Test%")
            ).all()
...
def setup_test_environment() -> Tuple[models.Organization, models.User, models.Token]:
    ...
    try:
        ...
        existing_org = db.query(models.Organization).filter(
            models.Organization.name == "Test Organization"
        ).first()
        
        if existing_org:
            print(f"ℹ️ Test organization already exists: {existing_org.name} (ID: {existing_org.id})")
...
if __name__ == "__main__":
    ...
    if command == "setup":
        try:
            ...
            print(f"✅ Setup completed - Organization: {org.id}, User: {user.id}, Token: {token.id}")
        except Exception as e:
