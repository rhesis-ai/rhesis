...
`a_generate(...)` automatically retries transient language-model failures. The retry policy covers
connection errors, timeouts, HTTP `429`, and HTTP `500` / `502` / `503` / `504` responses. The SDK
makes one initial attempt plus up to three retries with exponential jitter. Client-side errors such
as HTTP `400`, `401`, `403`, and `404` are not retried.
