...
Typical issues: wrong `DB_HOST`/`APP_DB_PASS` or Postgres not running, missing `.env` keys, wrong `PYTHONPATH`/venv, or JWT/auth misconfiguration. Check logs and [Environment configuration](./environment-config).
