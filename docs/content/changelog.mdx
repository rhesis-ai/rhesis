...
This release introduces **Parameters & Experiments**, expands **Test Explorer**, adds Enterprise Edition SSO and API Clients, and moves file bytes out of Postgres into object storage. It also hardens Polyphemus adversarial generation and updates SDK connector paths for parameter and file-aware execution.

### Featured Capabilities

**Parameters & Experiments**

- Project-scoped parameter schemas with typed values and defaults
- Versioned experiments with sequential version identifiers such as `v1`, `v2`, and `v3`
- Environment-based parameter routing, replacing the older "Labels" terminology
- Unified parameter injection with `{{ params.* }}` in endpoint request mappings
- Experiment selection in execution drawers, including multiple experiment-version pairs
- SDK support through `Parameters.get()`, `Experiment.run()`, and `TestSet.execute(experiment=...)`

**File storage and multimodal execution**

- File bytes are stored in object storage through `STORAGE_SERVICE_URI` (`file://`, `gs://`, or `s3://`)
- `GET /files/{id}/content` returns HTTP 302 to a presigned URL instead of streaming bytes directly
- `FileCreate` no longer accepts a `content` field; uploads stream bytes while Postgres stores metadata
- `File.extracted_text` is pre-computed at upload time
- `GET /files/{id}/thumbnail?size=N` serves WebP thumbnails at `72`, `144`, or `288` pixels
- `FileReference` lets SDK connector endpoints receive file metadata and fetch bytes on demand
- `extraction_status` on file responses reports `pending`, `done`, `failed`, or `not_applicable`
- ETag and If-None-Match support improves browser caching for content and thumbnails

**Enterprise authentication and feature gating**

- Per-organization OIDC SSO with slug-based login URLs and auto-provisioning controls
- API Clients for RFC 8693 token exchange from organization IdP tokens to Rhesis JWTs
- Central `FeatureRegistry` and `GET /features` endpoint for license-aware UI gating
- EE backend bootstrap keeps SSO and API Clients outside the Community import graph
...
- Adversarial system primer is injected on every Polyphemus request
- Schema-generation calls place the primer before schema instructions
- Batch item failures are logged and returned per item instead of silently disappearing
- Synthesizer `harmful=True` mode generates adversarial-only test cases
- The Polyphemus image is slimmer by depending on backend core instead of the full backend extras stack
