...
**Backend v0.10.0:**
- Fixed an issue where OAuth callbacks used the wrong base URL by separating the public API URL configuration (`
...
**Backend v0.9.0:**
- Introduced endpoint creation wizard with interactive test-and-map UI.
...
- Centralized configuration: unified environment detection under `BACKEND_ENV`, separated application from administrative database credentials.
- Fixed test execution reliability, tenant scope in Celery tasks, RLS migration superuser requirement, and metric evaluation failures for single-turn tests.

**Frontend v0.9.0:**
- Major UI redesign across all pages and components, aligned with the Figma design system.
...
**SDK v0.9.0:**
- Introduced `OWASPSynthesizer` for OWASP LLM Top 10 red-teaming.
- Refactored connector execution model with `EndpointContext`, replacing local backend execution.
- Fixed `system_prompt` stripping, event loop blocking in batch execution, and metric evaluation failures for single-turn tests.
...
**Backend v0.7.0:**
- Introduced Rhesis Architect, an AI agent for test suite design and execution.
- Implemented adaptive testing embeddings and diversity-aware suggestions.
- Added flexible model selection for execution and evaluation models.
- Enhanced telemetry exporter with retry logic and exponential backoff.
- Fixed Garak probe-coupled detectors, UUID conversion errors, and email notifications on test generation completion.

**Frontend v0.7.0:**
- Rhesis Architect: new AI agent interface for intelligent test suite design and execution.
- Adaptive Testing: navigation to embeddings and diversity-aware suggestion features.
- Model Selection: added support for flexible model selection and execution models.
- Fixed Docker connectivity issues by standardizing default endpoints to `host.docker.internal`.

**SDK v0.7.0:**
- Introduced Rhesis Architect agent for test suite design and execution.
- Added adaptive testing embeddings and diversity-aware suggestions.
- Added flexible model selection and execution model support.
- Enhanced telemetry exporter with retry and exponential backoff.
- Fixed Garak probe-coupled detectors, LiteLLM strict JSON schema handling, and patched vulnerable dependencies.
...
**Frontend v0.6.5:**
- Introduced AI-powered auto-configuration for endpoint mappings.
- Added evaluation of three-tier metrics during live multi-turn executions.
- Integrated a default Rhesis embedding model for new projects.
- Implemented duplication of endpoints, behaviors, and metrics.
- Introduced adaptive testing capabilities.
- Fixed test run metric filtering and endpoint creation bugs.
