...
def setup_test_environment(
    ...
    try:
        ...
        if test_db_url:
            original_env["SQLALCHEMY_DATABASE_TEST_URL"] = os.environ.get("SQLALCHEMY_DATABASE_TEST_URL")
...
def assert_test_database_used():
    ...
    assert SQLALCHEMY_DATABASE_URL == test_url, (
        f"Expected to use test database {test_url}, "
        f"but using {SQLALCHEMY_DATABASE_URL}"
    )
...
def get_test_database_stats() -> dict:
    ...
    return {
        ...
        "is_cloud_sql": "/cloudsql/" in SQLALCHEMY_DATABASE_URL or "/tmp/cloudsql/" in SQLALCHEMY_DATABASE_URL,
