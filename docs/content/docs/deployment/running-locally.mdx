...
**Note:** The `./rh start` command automatically sets `QUICK_START=true`. If you're manually configuring, ensure `QUICK_START=true` is set in your `.env.docker.local` file, then restart services with `./rh restart`.
