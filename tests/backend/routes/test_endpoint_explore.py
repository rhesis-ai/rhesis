...
class TestExploreEndpointHappy:
    def test_returns_task_id(self, app, endpoint_id, mock_user):
        ...
        with (
            patch("rhesis.backend.app.routers.endpoint.endpoint_crud") as mock_endpoint_crud,
            patch("rhesis.backend.app.routers.endpoint.launch_job", return_value=mock_async_result),
        ):
            mock_endpoint_crud.get_endpoint.return_value = mock_ep
...
class TestExploreEndpoint404:
    def test_returns_404_for_unknown_endpoint(self, app, endpoint_id):
        with patch("rhesis.backend.app.routers.endpoint.endpoint_crud") as mock_endpoint_crud:
            mock_endpoint_crud.get_endpoint.return_value = None
