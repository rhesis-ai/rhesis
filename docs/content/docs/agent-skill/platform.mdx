...
The **platform skill** (`rhesis`) packages the platform's domain knowledge into a portable skill file. Once installed, your AI interface can explore endpoints, design test suites, generate and run tests, and analyze results using the Rhesis MCP server as its backend.
...
If you have a product requirements document and want a **test foundation** on Rhesis before running tests, use the [PRD skill](/docs/agent-skill/prd). Paste your PRD with `/rhesis-prd` to create behaviors, custom metrics, tags, and generated test sets via MCP — then use this platform skill to execute and analyze.

## Platform skill vs. native Architect

The Rhesis platform has a built-in Architect agent with a web chat UI. This skill is a complement, not a replacement:

| | Native Architect | Platform skill |
|---|---|---|
...
See also the [PRD skill](/docs/agent-skill/prd) (`skills/rhesis-prd/`) for building a test foundation from requirements.
