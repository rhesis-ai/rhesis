...
def upgrade() -> None:
    ...
    if total_endpoints == 0:
        ...
        while offset < total_endpoints:
            ...
            for endpoint in endpoints:
                ...
                if needs_update:
                    set_clause = ", ".join(
                        [f"{field} = :{field}" for field in update_fields.keys()]
                    )
                    update_fields["endpoint_id"] = str(endpoint_id)

                    connection.execute(
                        text(
                            f"UPDATE endpoint SET {set_clause}, updated_at = NOW() WHERE id = :endpoint_id"
                        ),
                        update_fields,
                    )
                    encrypted_count += 1
            ...
            offset += BATCH_SIZE
            print(
                f"  Progress: {min(offset, total_endpoints)}/{total_endpoints} endpoints processed"
            )

        print(
            f"  ✓ Complete: {encrypted_count} encrypted, {skipped_count} skipped (already encrypted)"
        )
        ...
        while offset < total_models:
            ...
            for model in models:
                ...
                if api_key and not is_encrypted(api_key):
                    ...
                    connection.execute(
                        text(
                            "UPDATE model SET key = :encrypted_key, updated_at = NOW() WHERE id = :model_id"
                        ),
        ...
        while offset < total_tokens:
            ...
            for token_record in tokens:
                ...
                if token_value and not is_encrypted(token_value):
                    ...
                    connection.execute(
                        text(
                            "UPDATE token SET token = :encrypted_token, updated_at = NOW() WHERE id = :token_id"
                        ),
    ...
    plaintext_endpoints = connection.execute(
        text("""
    ...
    plaintext_models = connection.execute(
        text("""
    ...
    plaintext_tokens = connection.execute(
        text("""
...
def downgrade() -> None:
    ...
    endpoints = connection.execute(
        text("SELECT id, auth_token, client_secret, last_token FROM endpoint")
    ).fetchall()
    ...
    for endpoint in endpoints:
        ...
        if client_secret and is_encrypted(client_secret):
            try:
                ...
                print(
                    f"  ⚠ Warning: Could not decrypt client_secret for endpoint {endpoint_id}: {e}"
                )
        ...
        if update_fields:
            ...
            update_fields["endpoint_id"] = str(endpoint_id)
            connection.execute(
                text(f"UPDATE endpoint SET {set_clause} WHERE id = :endpoint_id"), update_fields
            )
            decrypted_count += 1
