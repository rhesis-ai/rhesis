...
> **Note:** For comprehensive chord management, see [Chord Management and Monitoring](chord-management.md)

The current test-execution path uses the async batch engine (single Celery task per run with
internal asyncio fan-out). Chord troubleshooting below is kept for historical incidents and
older deployments.

For current execution behavior and diagnostics, see:

- [Test Execution](test-execution.md)
- [Execution Modes](execution-modes.md)
- [Background Tasks](background-tasks.md)

### Quick Chord Issue Resolution (Legacy)
...
> **Tip:** See [Chord Management Guide](chord-management.md) for comprehensive solutions
...
- Current path: async batch execution issues (cancellation/retry edge cases, worker outages)
- Legacy path: chord callback (`collect_results`) never executes

**Solution**:
1. Check active Celery tasks: `celery -A rhesis.backend.worker.app inspect active`
2. Review worker logs for batch runner events and revoke/cancellation messages
3. Confirm run status transitions for `Progress` to terminal status (`Completed`, `Failed`, `Partial`, `Cancelled`)
4. If this is a historical deployment that used chord fan-out, check chord state with:
