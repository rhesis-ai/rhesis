...
<CodeBlock filename="base_task.py" language="python">
...
<CodeBlock filename="tenant_context_decorator.py" language="python">
...
<CodeBlock filename="count_test_sets.py" language="python">
...
<CodeBlock filename="task_launcher.py" language="python">
...
<CodeBlock filename="task-structure.txt" language="text">
{`tasks/
├── __init__.py
├── base.py
├── example_task.py
...
<CodeBlock filename="echo_task.py" language="python">
...
<CodeBlock filename="get_test_configuration.py" language="python">
...
<CodeBlock filename="manual_db_example.py" language="python">
...
<CodeBlock filename="Dockerfile" language="bash">
...
The worker startup script applies these configurations. Celery reads its log
level from the same `LOG_LEVEL` variable used by the rest of the backend,
rather than a separate Celery-specific variable:
...
<CodeBlock filename="task_status.py" language="python">
...
`BaseTask` logs with tenant context, retries with backoff, persists terminal errors in the result backend, and runs success/failure hooks when configured.

## Troubleshooting

### Dealing with Stuck Tasks

Long-running work (including test runs) is mostly a **single Celery task** that runs an async batch inside the worker. Tasks can stall or retry when the broker connection drops, the worker is killed mid-run, or time limits are hit. For test execution specifically, see [Test execution](../worker/test-execution.mdx) and [Worker troubleshooting](../worker/troubleshooting.mdx).

#### Inspecting and revoking tasks
...
Broker and worker tuning live in `apps/backend/src/rhesis/backend/celery/config.py` (for example `task_annotations` time limits on execution tasks).
...
<CodeBlock filename="Terminal" language="text" isTerminal={true}>
