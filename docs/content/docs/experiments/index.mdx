...
Production code shouldn't hardcode version hashes — it asks for `environment="production"`.
...
Rhesis provides three well-known environment names out of the box: `default`, `production`, and `staging`.

- `default` is the implicit pointer. It's what the SDK returns when no environment is explicitly requested.
- `production` and `staging` enable deployment-style fan-out without needing to copy experiments between projects.
...
For the most reproducible production deployments, you can *pin* a version in the SDK (`version="v_abc"`) to bypass environment resolution entirely.
