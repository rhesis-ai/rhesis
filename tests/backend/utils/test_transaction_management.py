...
class TestCRUDUtilsTransactionManagement:
    ...
    def test_create_db_item_commits_on_success(self, test_db: Session):
        ...
        organization, user, _ = create_test_organization_and_user(
            test_db, f"Transaction Test Org {unique_id}", f"transaction-user-{unique_id}@test.com", "Transaction User"
        )
        ...
        db_behavior = test_db.query(models.Behavior).filter(
            models.Behavior.id == result.id
        ).first()
    ...
    def test_create_db_item_with_commit_false_still_persists(self, test_db: Session):
        ...
        db_topic = test_db.query(models.Topic).filter(
            models.Topic.id == result.id
        ).first()
    ...
    def test_update_item_commits_on_success(self, test_db: Session):
        ...
        db_behavior = test_db.query(models.Behavior).filter(
            models.Behavior.id == behavior.id
        ).first()
        assert db_behavior is not None
    ...
    def test_delete_item_commits_on_success(self, test_db: Session):
        ...
        db_category = test_db.query(models.Category).filter(
    ...
    def test_get_or_create_entity_returns_existing(self, test_db: Session):
        ...
        topic_count = test_db.query(models.Topic).filter(
    ...
    def test_multiple_operations_transaction_isolation(self, test_db: Session):
        ...
        db_behavior1 = test_db.query(models.Behavior).filter(
            models.Behavior.id == result1.id
        ).first()
        db_behavior2 = test_db.query(models.Behavior).filter(
            models.Behavior.id == result2.id
        ).first()
    ...
    def test_update_item_with_invalid_id_returns_none(self, test_db: Session):
        ...
        phantom_behavior = test_db.query(models.Behavior).filter(
            models.Behavior.id == non_existent_id
        ).first()
    ...
    def test_delete_item_with_invalid_id_returns_none(self, test_db: Session):
        ...
        all_categories = test_db.query(models.Category).filter(
            models.Category.organization_id == organization.id
        ).count()
