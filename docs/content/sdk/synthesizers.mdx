...
synthesizer = Synthesizer(
    ...
    requirements=["helpful", "refuses harmful requests", "admits uncertainty"],
...
config = GenerationConfig(
    ...
    requirements=["accurate", "cites sources"],
...
`OWASPSynthesizer` always creates harmful tests. It spreads `num_tests` across the
selected OWASP categories and tags each test with `metadata["owasp_category"]` and
`metadata["owasp_name"]`. Pass `test_type=TestType.MULTI_TURN`
(`from rhesis.sdk.enums import TestType`) for conversational attacks instead of
one-shot prompts.
...
| `test_type` | `TestType.SINGLE_TURN` | `SINGLE_TURN` generates one-shot prompts; `MULTI_TURN` generates conversational attacks |

Use the Agentic Top 10 report by changing both the report and the requirement label:

<CodeBlock filename="owasp_agentic_synthesizer.py" language="python">
