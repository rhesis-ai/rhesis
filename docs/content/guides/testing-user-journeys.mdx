...
| `Actions / decisions` | What the user and assistant do each step | Captured in the multi-turn conversation history inside a `Test Run` |
...
| `Emotions / UX quality` | Clarity, trust, frustration, confidence | A journey-specific `Behavior` + `Metrics` (e.g., clarity/confirmation/empathetic tone) |
...
**Additional testing controls (not part of journey maps):**
- **Instructions**: Optional guidance for how the test agent should conduct the conversation
- **Restrictions**: Optional boundaries defining what the system must not do during testing
...
### Step 2: Create 1-2 behaviors for this flow
...
### Step 3: Create 5 multi-turn tests per behavior

Each test focusses on the same flow slice `Flight booking (happy path)`, but tests for a different behavior so your evaluation is done for one expectation at a time.
...
Execute the test set against the target endpoint. When you look at outcomes, separate these questions:
