...
export class TestConfigurationsClient extends BaseApiClient {
  async getTestConfigurations(params: Partial<PaginationParams> = {}): Promise<PaginatedResponse<TestConfigurationDetail>> {
  ...
  async getTestConfiguration(id: string): Promise<TestConfigurationDetail> {
    return this.fetch<TestConfigurationDetail>(`${API_ENDPOINTS.testConfigurations}/${id}`);
  }

  async createTestConfiguration(testConfiguration: TestConfigurationCreate): Promise<TestConfiguration> {
  ...
  async updateTestConfiguration(id: string, testConfiguration: TestConfigurationUpdate): Promise<TestConfiguration> {
    return this.fetch<TestConfiguration>(`${API_ENDPOINTS.testConfigurations}/${id}`, {
      method: 'PUT',
      body: JSON.stringify(testConfiguration),
    });
  }
  ...
  async executeTestConfiguration(id: string): Promise<TestConfigurationExecuteResponse> {
    return this.fetch<TestConfigurationExecuteResponse>(`${API_ENDPOINTS.testConfigurations}/${id}/execute`, {
      method: 'POST'
    });
  }

  async getTestRunsByTestConfiguration(id: string, params: Partial<PaginationParams> = {}): Promise<PaginatedResponse<TestRunDetail>> {
    const paginationParams = { ...DEFAULT_PAGINATION, ...params };
  ...
  async getLatestTestRun(id: string): Promise<TestRunDetail | null> {
    const response = await this.getTestRunsByTestConfiguration(id, { limit: 1 });
    return response.data.length > 0 
      ? this.fetch<TestRunDetail>(`${API_ENDPOINTS.testRuns}/${response.data[0].id}`)
      : null;
  }
} 