...
from rhesis.backend.jobs.utils import format_execution_time, format_execution_time_from_ms
...
def inject_annotation_counts_into_serialized_runs(
    serialized_runs: list[dict],
    annotation_stats: Dict[str, Dict[str, int]],
) -> None:
    """Merge per-run annotation aggregates into each run's ``counts`` dict."""
    empty = {"annotated_tests": 0, "corrected_tests": 0}
    for item in serialized_runs:
        bucket = annotation_stats.get(str(item.get("id")), empty)
