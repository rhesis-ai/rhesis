...
class OrganizationFilterMiddleware:
    ...
    def apply_organization_filter(self, query: Union[Query, Select], model_class: Type) -> Union[Query, Select]:
        """
        ...
        if (hasattr(model_class, '__name__') and 
            model_class.__name__ in self.organization_models and
            hasattr(model_class, 'organization_id')):
            ...
            try:
                ...
                logger.error(f"Invalid organization_id format: {self._current_organization_id}, error: {e}")
    ...
    def query(self, *args, **kwargs):
        ...
        if len(args) == 1 and hasattr(args[0], '__name__'):
            ...
            if (self.organization_id and 
                model_class.__name__ in organization_filter.organization_models and
                hasattr(model_class, 'organization_id')):
                ...
                try:
                    ...
                    logger.error(f"Invalid organization_id format: {self.organization_id}, error: {e}")
...
def get_organization_aware_session(session: Session, organization_id: Optional[str] = None) -> SessionWithOrganizationFilter:
    """
            ...
            if "organization_id" in statement:
                ...
                for model_name in organization_filter.organization_models:
                    ...
                    if table_name in statement.lower():
                        logger.warning(f"⚠️  Query on {model_name} table without organization filtering: {statement[:100]}...")
                        break
...
def configure_organization_filtering(
    enabled: bool = False,
    log_unfiltered_queries: bool = True,
