...
class TestTagOrganizationSecurity:
    ...
    def test_get_tag_organization_filtering(self, test_db: Session):
        ...
        org1, user1, _ = create_test_organization_and_user(
            test_db, f"Tag Org 1 {unique_id}", f"tag-user1-{unique_id}@security-test.com", "Tag User 1"
        )
        org2, user2, _ = create_test_organization_and_user(
            test_db, f"Tag Org 2 {unique_id}", f"tag-user2-{unique_id}@security-test.com", "Tag User 2"
        )
    ...
    def test_delete_tag_organization_filtering(self, test_db: Session):
        ...
        tag2 = crud.create_tag(test_db, tag_data2, organization_id=str(org1.id), user_id=str(user1.id))
...
class TestTestSetOrganizationSecurity:
    ...
    def test_delete_test_set_organization_filtering(self, test_db: Session):
        ...
        test_set2 = crud.create_test_set(test_db, test_set_data2, organization_id=str(org1.id), user_id=str(user1.id))
...
class TestServiceSecurityValidation:
    ...
    def test_service_cross_tenant_isolation(self, test_db: Session):
        ...
        unique_id = str(uuid.uuid4())[:8]
        org1, user1, _ = create_test_organization_and_user(
            test_db, f"Isolation Org 1 {unique_id}", f"isolation-user1-{unique_id}@security-test.com", "Isolation User 1"
        )
        org2, user2, _ = create_test_organization_and_user(
            test_db, f"Isolation Org 2 {unique_id}", f"isolation-user2-{unique_id}@security-test.com", "Isolation User 2"
        )
        ...
        tag_org1 = crud.create_tag(test_db, tag_data1, organization_id=str(org1.id), user_id=str(user1.id))
        
        # Create tag in org2 with same name
        tag_data2 = TagDataFactory.sample_data()
        tag_data2["name"] = tag_name
        tag_org2 = crud.create_tag(test_db, tag_data2, organization_id=str(org2.id), user_id=str(user2.id))
