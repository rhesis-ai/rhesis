...
{`GET /requirements?$filter=contains(name,'Test')
GET /requirements?$filter=startswith(name,'Test')
GET /requirements?$filter=endswith(name,'Requirement')

# Case-insensitive search with tolower / toupper
GET /requirements?$filter=contains(tolower(name),'test')`}
</CodeBlock>
...
{`GET /tests?%24filter=contains(requirement%2Fname,%27Test%27)`}
</CodeBlock>
...
Use function-style syntax for string operations — `contains(requirement/name,'test')`, not
`requirement/name contains 'test'`.

## Endpoints supporting OData filtering

`/tests`, `/requirements`, `/topics`, `/categories`, `/test_sets`, `/test_runs`, `/test_results`,
