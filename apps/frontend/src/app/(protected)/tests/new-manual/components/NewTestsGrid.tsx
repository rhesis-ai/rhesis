...
export default function NewTestsGrid({ onSave, onCancel }: NewTestsGridProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setBehaviors(behaviorsData.filter(b => b.id && b.name && b.name.trim() !== ''));
        ...
        notifications.show('Failed to load behaviors and topics', { severity: 'error' });
      }
    };
    fetchData();
  ...
  const columns: GridColDef[] = [
    { 
      ...
      valueGetter: (params: { row: GridRowModel }) => {
        ...
        const behavior = behaviorOptions.find(b => b.id === params.row.behaviorId);
      ...
      renderEditCell: (params) => (
        <Autocomplete
          ...
          value={behaviorOptions.find(opt => opt.id === params.row.behaviorId) || null}
          onChange={(_, newValue) => {
            if (newValue) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      notifications.show(error instanceof Error ? error.message : 'Failed to save tests', { severity: 'error' });
    }
  };
