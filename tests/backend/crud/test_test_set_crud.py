...
class TestTestSetSoftDeleteContract:
    ...
    def test_get_test_set_raises_for_deleted(
        ...
        with pytest.raises(ItemDeletedException):
            test_set_crud.get_test_set(test_db, test_set_id, organization_id=test_org_id)
    ...
    def test_get_test_set_by_nano_id_or_slug_raises_for_deleted(
        ...
        with pytest.raises(ItemDeletedException):
            test_set_crud.get_test_set_by_nano_id_or_slug(
                test_db, slug, organization_id=test_org_id
            )

    def test_get_test_set_returns_none_for_nonexistent(self, test_db: Session, test_org_id: str):
        import uuid

        result = test_set_crud.get_test_set(test_db, uuid.uuid4(), organization_id=test_org_id)
...
class TestUpdateTestSetAttributesSoftDeleteHandling:
    ...
    Regression test: test_set_crud.get_test_set now raises ItemDeletedException instead of
