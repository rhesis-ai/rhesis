...
interface CreateApiClientDialogProps {
   ...
   * persist across re-mounts (e.g. if the dialog briefly closes and
   * re-opens).
   */
...
export default function CreateApiClientDialog({
  open,
  onCancel,
  onSubmit,
  errorMessage,
  ...
  const handleScopeToggle = (scope: string) => {
    setForm(prev => {
      ...
      // If removing the currently-default scope, clear it so the
      // user must re-pick. Mirrors the backend's "default in allowed"
      // rule without silently fixing the value behind their back.
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // Strip empty optional fields so the backend treats them as
      // unset rather than zero-length strings.
        ...
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          >
            ...
            <Select
              ...
              onChange={e => setForm({ ...form, default_scope: String(e.target.value) })}
            >
