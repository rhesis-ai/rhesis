"""Terminal chat REPL for Visit-Prep (no Rhesis tracing).

For traced interactive chat, use ``chat_traced.py`` instead.

Run from anywhere inside the visit-prep project:
...
Or from this folder (uses the parent pyproject + .env):
    ...
    cd agents/visit-prep/chat_terminal
...
# Resolve visit-prep project root (parent of this folder) before importing visit_prep.
...
BANNER = """
Visit-Prep — doctor visit preparation assistant
─────────────────────────────────────────────
