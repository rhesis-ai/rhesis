...
1. **Zero-Configuration Local Development** - Get started immediately with pre-configured defaults
...
# Edit .env.docker.local and update RHESIS_API_KEY with your key from https://app.rhesis.ai/
# Or use the default configuration as-is for immediate testing

# 3. Start all services (no file copying needed!)
...
- Run all migrations
- Create the default admin user (`admin@local.dev`)
- Enable auto-login for the frontend
...
<FileTree
  title="Local Development Files"
  data={[
    {
      name: '.env.docker.local',
      type: 'file',
      description: 'Pre-configured environment file (ready to use, no copying needed!)'
    },
    {
...
The `.env.docker.local` file includes:

**Authentication & Security (Development Defaults):**
- Dummy Auth0 credentials (not used due to local auth bypass)
- Pre-generated JWT secrets
- Local authentication enabled
- Auto-login functionality

**Database & Redis:**
- PostgreSQL: `rhesis-user` / `rhesis-local-password`
...
<Table
  ...
  rows={[
    ...
    ['Default Admin', 'admin@local.dev', 'Created automatically'],
  ]}
...
By default, the local setup uses the Rhesis API for test generation. To enable this:

1. Get your Rhesis API key from https://app.rhesis.ai/
2. Open `.env.docker.local` and update:
...
Alternatively, configure other AI providers (optional):
...
# OpenAI (uncomment and configure)
