...
import { ConnectedModelCard, PlatformKeyDrawer } from '.';
...
export interface ModelsPageInitialData {
  models: Model[];
...
interface ModelsPageClientProps {
  ...
  initialData?: ModelsPageInitialData;
}
...
export default function ModelsPageClient({
  ...
  const [statusOptions, setStatusOptions] = useState<string[]>(() =>
    initialData?.statuses
      ? filterUniqueValidOptions(initialData.statuses).map(s => s.name)
      : []
  );
  ...
  useEffect(() => {
    ...
    loadData();
  }, [initialData, queryClient, userScope, status]);
