...
class TemplateService:
    ...
    def render_task_completion_text(
        ...
        execution_time_text = (
            TEXT_EXECUTION_TIME.format(execution_time=execution_time) if execution_time else ""
        )
        test_run_id_text = TEXT_TEST_RUN_ID.format(test_run_id=test_run_id) if test_run_id else ""
        error_details_text = (
            TEXT_ERROR_DETAILS.format(error_message=error_message)
            if (status.lower() == "failed" and error_message)
            else ""
        )
        view_results_text = (
            TEXT_VIEW_RESULTS.format(frontend_url=frontend_url, test_run_id=test_run_id)
    ...
    def render_task_completion_html(
        ...
        # Build conditional content
        execution_time_row = (
            HTML_EXECUTION_TIME_ROW.format(execution_time=execution_time) if execution_time else ""
        )
        test_run_id_row = (
            HTML_TEST_RUN_ID_ROW.format(test_run_id=test_run_id) if test_run_id else ""
        )
        error_details_section = (
            HTML_ERROR_DETAILS_SECTION.format(error_message=error_message)
            if (status.lower() == "failed" and error_message)
            else ""
        )
        view_results_section = (
            HTML_VIEW_RESULTS_SECTION.format(frontend_url=frontend_url, test_run_id=test_run_id)
            if (frontend_url and test_run_id)
            else ""
        )
    ...
    def render_test_execution_summary_html(
        ...
        if endpoint_name:
            endpoint_display = (
                f"{endpoint_name} ({endpoint_url})" if endpoint_url else endpoint_name
            )
            endpoint_info_row = f"""
            <tr>
