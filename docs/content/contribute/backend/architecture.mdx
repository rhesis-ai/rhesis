...
FastAPI app with routers, services, SQLAlchemy models, and OpenAPI docs out of the box.
...
`DEV_MODE=true` enables Uvicorn hot reload and `--log-level debug` in `start.sh`. The Celery worker reads its log level from the shared `LOG_LEVEL` variable rather than from `DEV_MODE`. Set `DEV_MODE=true` in `apps/backend/.env` when running via `./rh dev`; leave it unset or `false` everywhere else (Docker Compose local, cloud deployments).
