...
<Table
  ...
  rows={[
    ...
    ['`RHESIS_API_KEY`', 'Optional', 'API key for Rhesis-hosted models. Required when `DEFAULT_GENERATION_MODEL` or `DEFAULT_EVALUATION_MODEL` is set to `rhesis/rhesis-default`. Get your key from https://app.rhesis.ai/'],
    ['`DEFAULT_GENERATION_MODEL`', 'Default: `rhesis/rhesis-default`', 'Default model for test generation (format: provider/model_name). When set to `rhesis/rhesis-default`, requires `RHESIS_API_KEY`. Change to a third-party provider (e.g., `openai/gpt-4o`, `gemini/gemini-2.0-flash`) if not using a Rhesis API key'],
    ['`DEFAULT_EVALUATION_MODEL`', 'Default: `rhesis/rhesis-default`', 'Default model for evaluation/language-model-as-a-judge (format: provider/model_name). When set to `rhesis/rhesis-default`, requires `RHESIS_API_KEY`. Change to a third-party provider (e.g., `openai/gpt-4o`, `gemini/gemini-2.0-flash`) if not using a Rhesis API key'],
    ['`DEFAULT_EMBEDDING_MODEL`', 'Default: `vertex_ai/text-embedding-005`', 'Default model for embeddings (format: provider/model_name)'],
...
<Table
  ...
  rows={[
    ...
    ['`GOOGLE_CLIENT_SECRET`', 'Optional', 'Google OAuth client secret'],
    ['`GH_CLIENT_ID`', 'Optional', 'GitHub OAuth client ID. Enables GitHub sign-in'],
    ['`GH_CLIENT_SECRET`', 'Optional', 'GitHub OAuth client secret'],
...
<Table
  ...
  rows={[
    ...
    ['`VERTEX_AI_PROJECT`', 'Optional', 'Google Cloud project ID for Vertex AI'],
    ['`DEFAULT_GENERATION_MODEL`', 'Default: `rhesis/rhesis-default`', 'Default model for test generation (format: provider/model_name). When set to `rhesis/rhesis-default`, requires `RHESIS_API_KEY`. Change to a third-party provider (e.g., `openai/gpt-4o`, `gemini/gemini-2.0-flash`) if not using a Rhesis API key'],
    ['`DEFAULT_EVALUATION_MODEL`', 'Default: `rhesis/rhesis-default`', 'Default model for evaluation (format: provider/model_name). When set to `rhesis/rhesis-default`, requires `RHESIS_API_KEY`. Change to a third-party provider (e.g., `openai/gpt-4o`, `gemini/gemini-2.0-flash`) if not using a Rhesis API key'],
    ['`DEFAULT_EMBEDDING_MODEL`', 'Default: `vertex_ai/text-embedding-005`', 'Default model for embeddings (format: provider/model_name)'],
    ['`STORAGE_SERVICE_URI`', 'Optional', 'Storage service endpoint URL for file operations'],
    ['`STORAGE_SERVICE_ACCOUNT_KEY`', 'Optional', 'Storage service account key or access token'],
    ['`LOCAL_STORAGE_PATH`', 'Optional', 'Local filesystem path for file storage'],
