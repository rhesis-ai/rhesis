...
`FileReference` objects. Extracted text from text files and PDFs is passed as a
text part. Images, audio, video, and other binary files become native multimodal
content blocks for the target framework.

Object-storage bytes are fetched only when needed. Async target execution uses
`aread_bytes()` so attachment downloads do not block the event loop. See
[Extending Penelope](/docs/tests/conversation-simulation/extending#async-and-file-aware-targets)
for target examples and [Connector File Attachments](/sdk/connector/files) for
the `FileReference` API.
