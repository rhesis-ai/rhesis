...
| `requirement` | `requirement_pass_rates` |
...
All UUID and tag filters accept multiple values via repeated parameters (e.g. `?requirement_ids=uuid1&requirement_ids=uuid2`); results match any of the given IDs. `tags` uses AND logic (a test must carry all listed tags).
...
| `requirement_ids` | UUID[] | Filter by requirements |
...
Each `*_pass_rates` block reports `total`, `passed`, `failed`, and `pass_rate` (percentage). `metric_pass_rates`, `requirement_pass_rates`, `category_pass_rates`, and `topic_pass_rates` key those blocks by name.
...
{`{
  ...
  "metadata": {
    ...
    "available_requirements": ["Factual Accuracy", "Reasoning"],
...
# Compare two requirements over a date range
curl '.../test_results/stats?mode=requirement&requirement_ids=uuid1&requirement_ids=uuid2&start_date=2024-01-01&end_date=2024-06-30' -H 'Authorization: Bearer TOKEN'`}
</CodeBlock>
