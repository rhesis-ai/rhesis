...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-tracing-overview.webp"
  darkSrc="/screenshots/rhesis-ai-tracing-overview-dark.webp"
  alt="Traces dashboard — trace list with operation, input, duration, span count and evaluation status"
/>
...
The dashboard is not the only way in. Recorded traces are also readable from code and from an agent:

- **Python SDK** — query traces, walk their spans, and annotate them. See [Traces & Spans](/sdk/entities/traces).
- **MCP** — `list_traces`, `get_trace`, `get_trace_metrics`, `lookup_span` and `list_trace_providers`, so an agent in Cursor or Claude Code can answer why a run failed or what it cost. See [Agent Skill](/docs/agent-skill).
...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-tracing-detail.webp"
  darkSrc="/screenshots/rhesis-ai-tracing-detail-dark.webp"
  alt="Trace detail — span hierarchy grouped by turn, with per-span timing and the span details panel"
/>
...
- [Traces & Spans](/sdk/entities/traces) - Read traces back from the SDK
