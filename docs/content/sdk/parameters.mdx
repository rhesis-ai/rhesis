...
@endpoint(
    ...
    request_mapping={
        "query": "{{ input }}",
        "model": "{{ experiment_parameters.model | default('gpt-4o') }}",
...
Parameters are rendered through the same Jinja2 template engine as `input`, `test_id`, and other platform variables. During a test run with an experiment, `experiment_parameters.model` resolves to the experiment's value. Without an experiment, the `default()` filter provides the fallback. The deprecated alias `params` still works but logs a warning.
...
If you only need the current test-run snapshot and do not want fallback API resolution, use `get_experiment_parameters()`:

<CodeBlock filename="current_snapshot.py" language="python">
{`from rhesis.sdk import get_experiment_parameters
