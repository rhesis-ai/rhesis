...
<Callout type="info">
  **The join is per-integration.** It groups turns that MAF itself served, and it does not coordinate with [`conversation_turn`](/docs/tracing/conversation-tracing#standalone-apps-owning-the-turn) or with another framework's integration. So a conversation whose turns alternate between a bare `agent.run()` and some other mechanism, under the same conversation id, arrives as more than one trace. Wrap every turn of such a conversation in `conversation_turn` to keep them together. What each integration can join differs too, so check its own page rather than assuming this page's behaviour applies everywhere.
</Callout>
