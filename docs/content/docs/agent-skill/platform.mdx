...
The **Rhesis agent skill** (`rhesis`) packages platform knowledge into one portable skill file: explore endpoints, design test suites, generate and run tests, and analyze results. For **building from a PRD first**, see the [PRD workflow](/docs/agent-skill/prd) — same install, same skill.
...
If you have a product requirements document and want a **test foundation** before running tests, paste your PRD in chat — the skill routes to the [PRD workflow](/docs/agent-skill/prd). After approval it creates behaviors, metrics, tags, and test sets via MCP; then continue here to execute and analyze.
...
See also the [PRD workflow](/docs/agent-skill/prd) (`skills/rhesis/references/prd/`) for building a test foundation from requirements.
