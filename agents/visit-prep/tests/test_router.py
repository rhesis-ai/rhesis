...
from visit_prep.agents.router import IntentRouter
...
def test_router_parses_greeting():
    ...
    result = router.run(message="Hi there", state=VisitPrepState())
...
def test_router_parses_health_concern():
    ...
    router = IntentRouter(generator=gen)
    result = router.run(message="My knee hurts", state=VisitPrepState())
