...
Apply to every page you create or edit. Docs describe the current product for a reader who wants
to do something — not to sell, reassure, or restate what the page structure already shows.
...
- **No decorative emojis** in prose or headings (✅ 🚀 📦 ℹ️) — use "Note:" / "Warning:" or a
  `Callout`. Emojis passed as component props (e.g. `NextStepCard` icons) are a deliberate design
  element and stay.
