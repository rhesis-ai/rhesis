...
<CodeBlock filename="sdk_connector.py" language="python">
...
@endpoint(
    ...
    request_mapping={
        ...
        "system_prompt": "{{ experiment_parameters.system_prompt | default(none) }}",
    },
    response_mapping={
        "output": "{{ response }}",
    },
...
<CodeBlock filename="production_resolve.py" language="python">
...
- `experiment_parameters` / `parameters`: The resolved `Dict[str, Any]` of experiment parameter values. Both field names are sent; `experiment_parameters` is preferred.
...
<CodeBlock filename="chatbot_endpoint.py" language="python">
{`@endpoint(
    name="chat",
    request_mapping={
        "message": "{{ input }}",
        "model": "{{ experiment_parameters.model | default(none) }}",
        "temperature": "{{ experiment_parameters.temperature | default(0.7) }}",
        "system_prompt": "{{ experiment_parameters.system_prompt | default(none) }}",
        "use_case": "{{ experiment_parameters.use_case | default('travel') }}",
    },
    response_mapping={
        "output": "{{ message }}",
...
<CodeBlock filename="chatbot_route.py" language="python">
