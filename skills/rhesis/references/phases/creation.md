...
8. `generate_test_set` — per set; `config.behaviors`, `generation_prompt`, `test_type`, optional `sources` (Single-Turn only)
9. Poll `get_job_status` until `SUCCESS`
10. `get_test_set` + `list_test_set_tests` spot-check
11. Summarize by name; offer execution
