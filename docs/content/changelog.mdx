...
Rhesis now supports project-scoped parameter schemas, versioned experiments, and environment-based parameter routing:

- Define typed parameter slots once per project and fill them through experiments.
- Commit immutable experiment versions with sequential labels such as `v1`, `v2`, and `v3`.
- Promote shared experiment versions to environments such as `default`, `development`, `staging`, and `production`.
- Resolve parameters through the SDK with `Parameters.get()` or inject them into endpoint request mappings with `\{\{ params.* \}\}`.
- Execute test sets against an experiment object, raw `experiment_id`, version pin, or environment.

**Test Explorer and Architect UX**

The Explorer workflow is now the primary surface for adaptive testing:

- Added Explorer services, schemas, clients, and UI for creating, importing, evaluating, and managing Explorer test sets.
- Added improved Test Clusters hover behavior and a tabbed tests/clusters panel.
- Enhanced Architect with live task-progress events, per-category plan summaries, typo-tolerant entity resolution, and guarded ready/blocked plan gates.

**Enterprise auth and feature gating**

Enterprise deployments gain org-scoped authentication controls:

- Added per-organization OIDC SSO with Keycloak support.
- Added API Clients and RFC 8693 token exchange for machine-to-machine integrations.
- Introduced a central backend `FeatureRegistry` and frontend `FeaturesProvider` / `FeatureGate` mirror for gated capabilities.

**File storage and multi-modal execution**

File bytes now live in object storage instead of Postgres `bytea`, with streaming upload/download semantics:

- `POST /files` streams directly to object storage and enforces 10 MB per file, 20 MB per entity, and 10 files per request.
- `GET /files/{id}/content` now returns HTTP 302 to a presigned URL instead of streaming bytes directly. HTTP clients that follow redirects, including the SDK and browsers, are unaffected.
- `FileCreate` no longer accepts a `content` field; metadata is stored in Postgres and bytes are stored behind `STORAGE_SERVICE_URI`.
- `File.extracted_text` is pre-computed at upload time, and `FileResponse.extraction_status` reports `pending`, `done`, `failed`, or `not_applicable`.
- The SDK connector now uses `FileReference` to pass metadata without embedding raw bytes until an endpoint explicitly materializes them.

**Polyphemus 0.3.0**

Polyphemus now hardens adversarial generation and batch behavior:

- Injects an adversarial system primer on every request, including direct generation and structured synthesizer calls.
- Places the primer before schema instructions for JSON-schema requests and prefixes schema-backed user prompts with `[GENERATE ADVERSARIAL ATTACK] `.
- Logs individual batch item failures and returns per-item `error` payloads instead of silently swallowing failures.
- Splits backend dependencies into core and optional extras so the Polyphemus API image stays lightweight.

### Operational Notes

- Set `STORAGE_SERVICE_URI` to `gs://`, `s3://`, or `file://` storage. For GCS presigned URLs, set `STORAGE_SERVICE_ACCOUNT_KEY`; the backend Cloud Run service account needs `roles/storage.objectAdmin` on the bucket.
- Frontend `/api/*` proxying now resolves `BACKEND_URL` at request time instead of through build-time rewrites. Container startup replaces `NEXT_PUBLIC_API_BASE_URL` and quick-start placeholders so one image works across environments.
- `DEFAULT_EMBEDDING_MODEL` now defaults to `rhesis/rhesis-embedding`.
- `rh start` can prompt interactively for `RHESIS_API_KEY` when it is missing or still a placeholder.
- Apply Alembic revision `9b25f353e9c8` to add storage columns, deploy the new code, then apply revision `f1e2d3c4b5a6` to drop the legacy `content` column.
