...
<Callout type="info">
  This page documents a legacy execution path. Since `v0.6.12`, test execution
  uses an async batch engine rather than Celery chord fan-out. Use this
  reference only when investigating historical environments that still depend on
  chord tooling.
</Callout>
