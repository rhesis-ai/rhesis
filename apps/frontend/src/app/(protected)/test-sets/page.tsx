...
export default async function TestSetsPage() {
  try {
      ...
      response.data.map(async (testSet) => {
        if (testSet.status_id) {
          try {
            ...
            const status = await statusClient.getStatus(testSet.status_id as string);
            return {
              ...testSet,
              status: status.name
            };
          } catch (error) {
            console.error(`Error fetching status for test set ${testSet.id}:`, error);
            return testSet;
          }
        }
        return testSet;
    ...
    return (
      <PageContainer title="Test Sets" breadcrumbs={[{ title: 'Test Sets', path: '/test-sets' }]}>
