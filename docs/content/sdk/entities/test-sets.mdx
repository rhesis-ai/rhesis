...
### Explorer / TestTree node metadata (v0.6.12+)

If you use the SDK `TestTree` APIs (module `rhesis.sdk.adaptive_testing`) against Explorer sessions, each `TestTreeNode` can carry
evaluation metadata in addition to `label` and `model_score`.

This is useful when you want to inspect per-metric outcomes programmatically after Explorer
evaluation runs.

| Field | Type | Description |
| --- | --- | --- |
| `metrics` | `dict \| None` | Per-metric evaluation details when available |
| `label` | `"" \| "topic_marker" \| "pass" \| "fail"` | Aggregate node label |
| `labeler` | `str` | Comma-separated metric names or source labeler |
| `model_score` | `float` | Aggregate score across evaluated metrics |

When present, `metrics` is keyed by metric name and typically includes:
...
<CodeBlock filename="adaptive_tree_node_metrics.py" language="python">
...
node = TestTreeNode(
    ...
    label="pass",
    labeler="answer_relevancy, jailbreak_resistance",
    model_score=0.92,
    metrics={
        "answer_relevancy": {
            "score": 0.95,
        ...
        "jailbreak_resistance": {
            "score": 0.89,
            "is_successful": True,
            "reason": "Model refused unsafe request",
        },
    },
)

print(node.metrics["jailbreak_resistance"]["reason"])`}
</CodeBlock>

<CodeBlock filename="inspect_adaptive_metrics.py" language="python">
{`from rhesis.sdk.adaptive_testing import TestTree
from rhesis.sdk.entities import TestSets

test_set = TestSets.pull(name="Adaptive Safety Set")
tree_data = TestTree.from_test_set(test_set)

for node in tree_data.get_tests():
    if node.metrics:
        for metric_name, metric_result in node.metrics.items():
            print(metric_name, metric_result["score"], metric_result["is_successful"])`}
</CodeBlock>
