...
<Callout type="info">
  Explorer replaces the older "adaptive testing" naming in the product UI and API clients. Existing Explorer test sets are still identified internally by the `Adaptive Testing` requirement marker.
</Callout>
