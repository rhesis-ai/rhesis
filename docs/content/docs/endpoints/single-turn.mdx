...
When tests include file attachments (images, PDFs, or other documents), the
`files` variable contains an array of file objects. Each provider expects files
in a different format, so Rhesis provides built-in filters to handle the
conversion:
