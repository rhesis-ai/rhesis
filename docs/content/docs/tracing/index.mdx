...
Rhesis Tracing provides comprehensive observability for your LLM applications built on the OpenTelemetry standard. It captures detailed traces of LLM calls, tool invocations, and retrieval operations with a semantic layer designed specifically for AI workloads.
...
For multi-turn tests and chat sessions, traces include conversation-aware navigation:

- **Trace type filters**: Filter by single-turn or multi-turn traces
- **Conversation tab**: When a trace has `conversation_id`, a dedicated Conversation view is shown
- **Turn navigation**: Move between turns in tree, sequence, and graph views
- **Graph playback controls**: Replay execution over time with turn markers and speed controls
- **Progressive graph labels**: Conversation edges are labeled by turn (for example `T1`, `T2`)

Use this view when debugging conversation flow, handoffs, and tool usage across turns.
