...
graph TB
    subgraph Setup[" "]
        Org["Organization<br/><small>Team workspace &<br/>data isolation</small>"]
        Project["Projects<br/><small>AI applications<br/>to test</small>"]
        Org --> Project
    end

    subgraph Config[" "]
        Endpoint["Endpoints<br/><small>API connections</small>"]
        Tests["Tests<br/><small>Prompts & inputs</small>"]
        Metrics["Metrics<br/><small>Evaluation criteria</small>"]
    end

    subgraph Execution[" "]
        TestSet["Test Sets<br/><small>Grouped tests</small>"]
        TestRun["Test Runs<br/><small>Execution snapshots</small>"]
        Results["Test Results<br/><small>Aggregate analytics</small>"]
        TestSet --> TestRun
        TestRun --> Results
    end
...
Your **[Organization](/docs/organizations)** forms the foundation of everything in Rhesis. It's the top-level container that provides data isolation from other organizations, manages your team through invitations and access control, and ensures all members can access shared resources.
...
**[Projects](/docs/projects)** organize testing work for different AI applications. Each project represents a specific application or testing initiative with its own endpoints, tests, and results.
...
**[Endpoints](/docs/endpoints)** bridge Rhesis to your AI application's API. They define exactly how to connect and communicate with your system, whether it uses REST or WebSocket protocols.
...
**[Tests](/docs/tests-generation)** represent individual prompts or inputs sent to your AI application. Each test includes the prompt itself, metadata describing what behavior you're testing and the topic it covers, and optionally an expected response for comparison.
...
The configuration of your AI system often involves more than just prompts—it includes model selection, temperature, retrieval strategies, and other variables. 

Rhesis manages these through a **[Parameter Schema](/docs/experiments/parameter-schema)**, which defines the typed configuration "knobs" your project supports. You can then author **[Experiments](/docs/experiments)** that bundle specific values for these parameters into immutable *versions*. By using movable *environments* (like `default` or `production`), your application code and test runs can fetch the right configuration automatically without hardcoding version identifiers.

## Test Sets

**[Test Sets](/docs/test-sets)** collect tests that execute together, functioning like test suites in traditional software development. Running tests one at a time doesn't scale when you have hundreds of scenarios. Test sets let you execute bulk testing with one click, create regression suites that catch breaking changes, build pre-deployment validation pipelines, and organize tests by feature or scenario.
...
A **[Test Run](/docs/test-runs)** captures the complete result of executing a test set against an endpoint. It's a snapshot preserving exactly what happened during that execution.
...
The **[Test Results](/docs/test-results)** dashboard aggregates data from multiple test runs to reveal trends and patterns. While test runs show individual execution snapshots, test results provide aggregate analytics across multiple executions.
...
**Configure the Basics**: Add [endpoints](/docs/endpoints) that connect to your AI system. Define [metrics](/docs/metrics) that evaluate quality based on your requirements. Generate or manually create [tests](/docs/tests-generation) that cover your use cases.

**Organize and Execute**: Add tests to [test sets](/docs/test-sets) that make sense for your workflow—maybe one set for smoke tests, another for comprehensive regression testing. Run test sets against endpoints, which creates [test runs](/docs/test-runs) with detailed results you can analyze.

**Analyze and Improve**: Review [test runs](/docs/test-runs) for detailed debugging of failures. Check the [test results](/docs/test-results) dashboard to understand quality trends. Compare runs to detect regressions before they reach production. Generate more tests or adjust metrics based on what you learn.
...
<Callout type="default">
  **Ready to Start?** Set up your first [Project](/docs/projects),
  configure an [Endpoint](/docs/endpoints), then generate
  [Tests](/docs/tests-generation) with AI to start validating your Gen
  AI application.
</Callout>
