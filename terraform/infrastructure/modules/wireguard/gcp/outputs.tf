...
output "server_public_key" {
  ...
  value       = data.external.server_key.result.public_key
  sensitive   = true
}
...
output "peer_configs" {
  ...
  value = {
    for peer in var.wireguard_peers : peer.identifier => {
      ip          = peer.ip
      private_key = data.external.peer_keys[peer.identifier].result.private_key
      ...
      config = templatefile("${path.module}/templates/client-config.tpl", {
        peer_ip           = peer.ip
        peer_private_key  = data.external.peer_keys[peer.identifier].result.private_key
        server_public_key = data.external.server_key.result.public_key
