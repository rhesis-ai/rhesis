"""Run the canned scenarios traced through deepset's upstream integration.
...
Then run:
    ...
    uv run python examples/run_scenarios_traced_upstream.py

For the native integration that ships in this repo, use ``run_scenarios_traced_native.py`` instead.
"""
...
try:
    ...
    raise SystemExit(
        ...
        "  uv pip install -e <path-to>/haystack-core-integrations/integrations/rhesis\n"
        "Or trace through the native integration with run_scenarios_traced_native.py."
    ) from exc

if __name__ == "__main__":
    sys.exit(run_traced_scenarios(RhesisTracing, label="upstream"))
