...
- **Conversations no longer start on two traces.** A stateful endpoint's first turn was recorded on a
  trace id the platform generated, separate from the one the endpoint's own SDK had already opened,
  so the first turn appeared apart from the rest of the conversation.
