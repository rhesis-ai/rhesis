...
Always returns behavior, category, and topic pass rates, plus overall pass rates.
`METRICS`, `TIMELINE`, `TEST_RUNS`, `BEHAVIOR_DETAIL`, and `IDS` raise
`NotImplementedError` -- each is either a join across test_result- and metric-grain
data, or a single `Insights(...).ids()` call not worth wrapping. `priority_min`/
`priority_max` raise `ValueError` -- `Insights` has no range-filter equivalent.
