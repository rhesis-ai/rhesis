...
class TestTracer:
    ...
    @patch("rhesis.sdk.telemetry.tracer.get_tracer_provider")
    def test_serialize_pydantic_models(self, mock_get_provider):
        """Test that Pydantic models are serialized using model_dump()."""
        ...
        def test_func(prompt: str) -> IntentResult:
            return IntentResult(intent="informational", confidence="high")

        tracer.trace_execution("test_func", test_func, ("What is insurance?",), {})

        # Verify output is serialized as dict from Pydantic model
        import json
        ...
        # Should be the dict representation of the Pydantic model
        assert parsed_result == {"intent": "informational", "confidence": "high"}
        assert result_str == '{"intent": "informational", "confidence": "high"}'
