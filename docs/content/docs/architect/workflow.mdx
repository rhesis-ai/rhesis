...
Architect proposes a structured plan: requirements to test, test sets to generate, metrics to evaluate with, and how they connect. It checks what already exists on the platform and reuses it where possible.

**What you see:** The plan panel appears between the chat and the input. Requirements and metrics are labelled **(reuse)**, **(improve)**, or **(new)**.
...
Architect creates entities in order: requirements first, then metrics, then requirement-metric links — and generates test sets last, once every prerequisite is in place. It reports each step as it completes.
...
**What you see:** The mode chip reads `executing`. When the run completes, Architect presents a structured summary: overall pass rate, requirement breakdown, metric breakdown, and notable failures with the evaluator's reasoning.
