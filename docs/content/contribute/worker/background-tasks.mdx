...
`celery/config.py` and auto-discovers tasks under `rhesis.backend.jobs`:
...
Tasks inherit from `BaseJob`, which adds retry settings and tenant-context management. It reads
...
<CodeBlock language="python" filename="jobs/base.py" isTerminal={false}>
{`class BaseJob(Task):
...
<CodeBlock language="python" filename="jobs/decorators.py" isTerminal={false}>
...
`launch_job` launches a task from a FastAPI route, pulling `organization_id` and `user_id` off
`current_user` so callers don't pass them explicitly:

<CodeBlock language="python" filename="jobs/launcher.py" isTerminal={false}>
{`def launch_job(task: Callable, *args: Any, current_user=None, **kwargs: Any):
...
<CodeBlock language="python" filename="jobs/test_configuration.py" isTerminal={false}>
{`@app.task(base=BaseJob, name="rhesis.backend.jobs.get_test_configuration")
@with_tenant_context
...
<CodeBlock language="python" filename="jobs/manual_example.py" isTerminal={false}>
{`@app.task(base=BaseJob, name="rhesis.backend.jobs.manual_db_example")
...
Launch jobs from a route with `launch_job`:

<CodeBlock language="python" filename="routers/test_configuration.py" isTerminal={false}>
{`from rhesis.backend.jobs import launch_job, execute_test_configuration
...
`BaseJob` logs exceptions with tenant context, retries failed tasks with exponential backoff up to
