...
<Callout type="info">
  The legacy `@endpoint(parameters=["model", "temperature"])` path and the
  `\{\{ params.* \}\}` template alias still work for older applications, but
  both emit deprecation warnings. Use `\{\{ experiment_parameters.* \}\}` in
  `request_mapping` for new endpoints.
</Callout>
