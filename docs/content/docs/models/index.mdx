...
Models in Rhesis are used for three primary purposes:
...
- **Execution (Multi-Turn)**: Drive multi-turn conversation simulations with [Penelope](/docs/tests/conversation-simulation), the agent that conducts automated conversations against your endpoints

You can configure default models for each purpose in the model settings, or override them per execution when running tests (see [Test Execution](/docs/test-execution)).
...
- **Rhesis Default**: Default Rhesis-hosted model
  - No API key required
  - Pre-configured for generation, evaluation, and execution
