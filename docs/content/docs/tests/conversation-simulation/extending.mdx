...
Abstraction for systems under test. Penelope ships with several target adapters:
...
target = LangGraphTarget(graph=my_graph)

# Microsoft Agent Framework (any agent exposing an async run() method)
from agent_framework import ChatAgent
from agent_framework.openai import OpenAIChatClient

agent = ChatAgent(chat_client=OpenAIChatClient(), instructions="You are helpful.")
target = MicrosoftAgentFrameworkTarget(agent, target_id="maf-bot")`}
...
<Callout type="default">
  **Next:** Check out [Examples](/conversation-simulation/examples) to see custom tools in
  action, or learn about [Configuration](/conversation-simulation/configuration) options.
</Callout>
