...
def get_or_create_status(session: Session, status_name: str, entity_type: str, organization_id: str = None) -> Optional[Status]:
    ...
    entity_type_query = session.query(TypeLookup).filter(
        ...
        TypeLookup.type_value == entity_type
    )
    if organization_id:
        entity_type_query = entity_type_query.filter(TypeLookup.organization_id == UUID(organization_id))
    ...
    if not entity_type_lookup:
        ...
        if organization_id:
            ...
            entity_type_data["user_id"] = UUID(organization_id)  # Use org_id as placeholder for user_id
    ...
    query = session.query(Status).filter(
        ...
        Status.entity_type_id == entity_type_lookup.id
    )
