...
``architect_chat_task``, so the SDK telemetry ``ContextVar`` trio has
to be bound here for multi-turn traces to share a ``trace_id``.  These
tests cover:
...
* ``_conversation_telemetry_context`` binding / clearing the SDK
...
from rhesis.backend.jobs.architect.telemetry import (
...
class TestLoadSessionTraceId:
    ...
    def test_swallows_db_errors(self, _mock_db):
        assert _load_session_trace_id(_VALID_SESSION_ID, "org", "user") is None
...
# ── _load_session_trace_id rejects non-UUID input gracefully ────────
...
def test_task_parks_conversation_output_when_trace_id_available():
    ...
    Without it the Conversation view shows only user messages and
    marks every turn Failed.
    """
    from rhesis.backend.app.services.architect.runner import ArchitectChatResult
    ...
    with (
        ...
        patch("rhesis.backend.jobs.architect.chat.architect_chat_task.run"),
    ):
