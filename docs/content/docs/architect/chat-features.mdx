...
As Architect creates each entity, the corresponding checkbox is ticked. The panel gives a live view of creation progress without scrolling through tool activity in the chat. Architect also tracks whether prerequisites are ready or blocked before it generates test sets, so the checklist reflects behaviors, metrics, and behavior-metric mappings before generation starts.
