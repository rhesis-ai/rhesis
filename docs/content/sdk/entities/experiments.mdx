...
An experiment is a named bundle of parameter values for a project. Each save creates an immutable version with a sequential label such as `v1`, `v2`, or `v3`.
...
| `latest_version` | `str` | Sequential label of the most recent version |
...
print(f"Version: {version['version']}")  # v1`}
</CodeBlock>
...
# Specific version by sequential label
v = exp.get_version("v2")
print(v["values"])`}
</CodeBlock>

`get_version()` also accepts the stored `content_hash` for compatibility and troubleshooting, but new code should pass the sequential `version` label returned by `commit()` or `list_versions()`.
...
| `get_version(version)` | `dict` | Single version by sequential label or content hash |
