...
export default function TestRunTestsGrid({
  testRunId,
  sessionToken,
}: TestRunTestsGridProps) {
  ...
  const behaviorColumns: GridColDef<TestResultDetail>[] = useMemo(
    () =>
      behaviors.map(behavior => ({
        ...
        valueGetter: (_, row) => {
          ...
          const allPassed =
            passedMetrics === totalMetrics && failedMetrics.length === 0;
