...
FastAPI routers under `app/routers/`, one resource family per module where possible. Paths use normal REST patterns unless noted below.
...
The full list lives in `apps/backend/src/rhesis/backend/app/auth/public_routes.py` and is mutable so EE features (or other plugins) can extend it from their bootstrap before `app.include_router` runs for their own routers.
