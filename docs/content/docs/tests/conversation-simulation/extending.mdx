...
<CodeBlock filename="endpoint_target.py" language="python">
{`from rhesis.penelope import EndpointTarget

target = EndpointTarget(endpoint_id="your-endpoint-id")`}
</CodeBlock>

<CodeBlock filename="langchain_target.py" language="python">
...
<CodeBlock filename="langgraph_target.py" language="python">
...
<CodeBlock filename="pydantic_ai_target.py" language="python">
...
<CodeBlock filename="google_adk_target.py" language="python">
...
adk_agent = Agent(
    ...
    instruction="Help customers resolve support requests.",
)
runner = Runner(
    agent=adk_agent,
    app_name="support",
...
target = GoogleADKTarget(runner, target_id="adk-bot", description="Customer support agent")

# A bare agent works too - the target wraps it in a Runner with an
# in-memory session service.
target = GoogleADKTarget(adk_agent, target_id="adk-bot")`}
</CodeBlock>

<CodeBlock filename="haystack_target.py" language="python">
{`from haystack.components.agents import Agent
from haystack.components.generators.chat import OpenAIChatGenerator
from rhesis.penelope import HaystackTarget

haystack_agent = Agent(
    chat_generator=OpenAIChatGenerator(model="gpt-4o-mini"),
    system_prompt="Help customers resolve support requests.",
    tools=[],
)
haystack_agent.warm_up()
target = HaystackTarget(haystack_agent, target_id="support-agent")

# A Pipeline needs to be told which component and socket the message goes to.
...
<CodeBlock filename="async_penelope.py" language="python">
