...
**Intelligent Multi-Turn Testing for LLM Applications**

Rhesis conversation simulation is powered by Penelope, an autonomous testing agent that executes complex, multi-turn test scenarios against conversational AI systems. Penelope combines sophisticated reasoning with adaptive, goal-driven testing strategies to thoroughly evaluate LLM applications across security, user experience, compliance, edge cases, and more.
...
<Callout type="info">
  **Ready to get started?** Check out the [Getting
  Started](/docs/conversation-simulation/getting-started) guide to install Penelope and run your
  first test.
</Callout>
