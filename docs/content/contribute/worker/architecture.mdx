...
from rhesis.backend.jobs.base import BaseJob   # Worker-specific task base class
...
3. Worker-specific code (`rhesis.backend.jobs` and `rhesis.backend.worker`)
...
4. Base task classes, task organization, and worker configuration belong in `rhesis.backend.jobs`
...
<CodeBlock language="python" filename="task_context_example.py" isTerminal={false}>
{`# In the backend API
