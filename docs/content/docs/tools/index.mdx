...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-tools.webp"
  darkSrc="/screenshots/rhesis-ai-tools-dark.webp"
  alt="Tools page — connected Notion, GitHub, Jira, Linear, GitLab and Asana workspaces"
/>

## Setting Up a Tool Connection

Every connection follows the same flow in Rhesis: on the **Tools** page, click **Add tool connection**, choose a provider from the grid, fill in the connection name, token, and any provider-specific fields listed below, then **Test the connection** and **Save**. Each provider below covers only how to generate its token and which extra fields it needs.
...
<details>
<summary id="trello"><img src="/integrations/providers/trello.svg" width="20" height="20" alt="" style={{ display: "inline", verticalAlign: "middle", marginRight: "6px" }} /> Trello</summary>

<ToolPurposeChip text="Export knowledge into Rhesis" />

1. Create or open a Power-Up at [trello.com/power-ups/admin](https://trello.com/power-ups/admin).
2. On the Power-Up's API key tab, generate an API key.
3. Next to the key, click the Token link to authorize and generate a token — it's shown only once, so copy it right away.
4. In Rhesis, paste the API key and the token.
</details>
