...
@app.task(base=BaseTask, name="rhesis.backend.tasks.generate_and_upload_test_set", bind=True, display_name="Generate and Upload Test Set")
@with_tenant_context
def generate_and_upload_test_set(self, synthesizer_type: str, num_tests: int = 5, batch_size: int = 20, db=None, **synthesizer_kwargs):
    """
    ...
    self.log_with_context('info', f"Starting generate_and_upload_test_set task", 
                         num_tests=num_tests, 
    ...
    try:
        # Update task state to show progress
        self.update_state(state="PROGRESS", meta={"status": "Initializing SDK and getting user tokens"})
        ...
        self.log_with_context('info', f"SDK configured", base_url=base_url)
        ...
        try:
            ...
            raise ValueError(f"Unsupported synthesizer type: {synthesizer_type}. Supported types: {', '.join(supported_types)}")
        ...
        if synth_type == SynthesizerType.PARAPHRASING and 'source_test_set_id' in synthesizer_kwargs:
            ...
            processed_kwargs.update({k: v for k, v in synthesizer_kwargs.items() if k != 'source_test_set_id'})
        else:
        ...
        self.log_with_context('info', f"Synthesizer initialized", 
                             synthesizer_type=synthesizer_type,
        ...
        self.log_with_context('info', f"Test set generated", 
        ...
        self.log_with_context('info', f"Test set uploaded successfully", 
