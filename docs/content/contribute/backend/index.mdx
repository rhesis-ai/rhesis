...
In local and test environments (`BACKEND_ENV` is `local` or `test`), `migrate.sh` skips the Postgres wait loop and database ownership step. Deployed environments run both before applying migrations.
