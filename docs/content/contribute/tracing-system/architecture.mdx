...
The `post_ingest_link` task itself (`jobs/telemetry/post_ingest.py`) performs test-result linking, conversation-id linking, input-file linking, and dispatches an enrichment → evaluation chain per root span. See [Worker: Trace Ingestion Pipeline](/contribute/worker/trace-ingestion-pipeline) for the full breakdown, including metric evaluation.
...
| `trace_metrics_status` | LLM-based metric evaluation status. Human verdicts are [annotations](/contribute/backend/annotations) |
