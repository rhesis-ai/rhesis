...
### Using the `get_language_model` function

The easiest way to use models is through the `get_language_model` function. When called without any arguments, this function returns the default Rhesis language model.
...
model = get_language_model("gemini")`}
...
model = get_language_model("gemini/gemini-2.0-flash")`}
...
model = get_language_model(provider="gemini", model_name="gemini-2.0-flash")`}
...
model = get_language_model("openai/gpt-4o-mini")
...
### Using the `get_embedding_model` function

The easiest way to use embedders is through the `get_embedding_model` function. When called without arguments, it returns the default Rhesis embedding model.

<CodeBlock filename="default_embedder.py" language="python">
{`from rhesis.sdk.models import get_embedding_model
...
embedder = get_embedding_model(provider="openai", model_name="text-embedding-3-large")`}
