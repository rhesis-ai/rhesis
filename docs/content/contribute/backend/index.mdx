...
- **Kubernetes (Helm chart)** — a `backend-migrate` Job runs `migrate.sh` before the backend Deployment rolls (ArgoCD PreSync / Helm pre-upgrade hook), and the backend pods start with `SKIP_MIGRATIONS=true`. Set `backend.migrations.job.enabled: false` in the chart values to run migrations at pod boot instead.
...
HTTP routes live under **`app/routers/`**, with Pydantic types in **`app/schemas/`** and SQLAlchemy models in **`app/models/`**. Shared database helpers sit in **`app/crud.py`**. Authentication and token handling are grouped under **`app/auth/`** (see [Authentication](/contribute/backend/authentication)). Celery-related code is split between **`jobs/`** (what runs in the worker), **`celery/`** (app wiring), and **`worker.py`** at the package root. Database migrations are Alembic revisions under **`src/rhesis/backend/alembic/`**, applied by **`migrate.sh`** (see above).
