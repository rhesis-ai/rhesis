...
| `RequirementSpec` | `name`, `description`, `reuse_status`, `existing_id`, `completed` |
| `TestSetSpec` | `name`, `description`, `num_tests`, `test_type`, `generation_prompt`, `requirements`, `categories`, `topics`, `completed` |
...
| `MappingSpec` | `requirement`, `metrics` (list of metric names), `completed` |

`reuse_status` is a `Literal["reuse", "improve", "new"]` on `RequirementSpec` and `MetricSpec`.
...
| `add_requirement_to_metric` | `CREATING` | `MAPPING` |
...
TOOL_REGISTRY["my_new_tool"] = ToolEntry(
    ...
    plan_category=PlanCategory.REQUIREMENT,  # or None
)`}
</CodeBlock>
