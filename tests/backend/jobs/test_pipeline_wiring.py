...
class TestExecuteTestWiring:
    ...
    async def test_no_provider_when_no_params(self):
        ...
        with (
            patch(
                "rhesis.backend.jobs.execution.test_execution.get_test_and_prompt",
                return_value=(mock_test, "prompt", "expected"),
            ),
            patch(
                "rhesis.backend.jobs.execution.test_execution.create_executor",
                return_value=mock_executor,
            ),
        ):
            from rhesis.backend.jobs.execution.test_execution import (
                execute_test,
            )
...
class TestOrchestrationPassthrough:
    ...
    def test_parallel_receives_params(self):
        ...
        with (
            patch(
                "rhesis.backend.jobs.execution.orchestration.get_test_set",
                return_value=mock_test_set,
            ),
            patch(
                "rhesis.backend.jobs.execution.orchestration.get_execution_mode",
            ) as mock_mode,
            patch(
                "rhesis.backend.jobs.execution.orchestration.execute_tests_as_batch",
                return_value={"total_tests": 1},
            ) as mock_batch,
        ):
            from rhesis.backend.jobs.enums import ExecutionMode
            from rhesis.backend.jobs.execution.orchestration import (
                execute_test_cases,
            )
    ...
    def test_sequential_receives_params(self):
        ...
        with (
            ...
            patch(
                "rhesis.backend.jobs.execution.orchestration.execute_tests_sequentially",
                return_value={"total_tests": 1},
            ) as mock_sequential,
        ):
            from rhesis.backend.jobs.enums import ExecutionMode
    ...
    def test_empty_test_set_raises_execution_error(self):
        ...
        with (
            ...
            patch(
                "rhesis.backend.jobs.execution.orchestration.count_test_set_tests",
                return_value=0,
            ),
        ):
            from rhesis.backend.jobs.execution.orchestration import (
