...
`BaseJob` sets `autoretry_for = (Exception,)`, so every failure retries by
...
@app.task(base=BaseJob, bind=True, name="tests.permanent_error_task")
def _permanent_error_task(self):
    raise ModelConfigurationError("embedding model is not served in this region")


@app.task(base=BaseJob, bind=True, name="tests.transient_error_task")
...
def record_retries(monkeypatch):
    ...
    monkeypatch.setattr(BaseJob, "retry", fake_retry, raising=False)
    return attempts
...
class TestPermanentErrorNoRetry:
    ...
    def test_dont_autoretry_for_is_declared_on_the_task(self):
        """Catches the wiring being dropped from BaseJob or the decorator."""
