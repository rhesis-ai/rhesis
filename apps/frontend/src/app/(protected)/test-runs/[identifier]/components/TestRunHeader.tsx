...
import { Box, Card, CardContent, Typography, Chip, Grid, useTheme } from '@mui/material';
...
export default function TestRunHeader({ testRun, testResults, loading = false }: TestRunHeaderProps) {
  const theme = useTheme();
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <SummaryCard
            ...
            subtitle={testRun.attributes?.started_at ? formatDate(testRun.attributes.started_at) : 'N/A'}
          ...
          >
            <CardContent sx={{ flexGrow: 1, p: 3 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                <Chip
                  label={stats.status === 'in_progress' ? 'In Progress' : 'Completed'}
                  color={stats.statusColor}
                  icon={stats.status === 'in_progress' ? <PlayCircleOutlineIcon /> : stats.statusColor === 'success' ? <CheckCircleOutlineIcon /> : <CancelOutlinedIcon />}
                  size="medium"
                  sx={{ fontWeight: 600 }}
                />
              </Box>
              ...
              {testRun.test_configuration?.endpoint?.id ? (
                ...
                >
                  ...
                  >
                    ...
                    >
                      Endpoint: {testRun.test_configuration.endpoint.name || testRun.attributes?.environment || 'development'}
                    </Typography>
                    <OpenInNewIcon
                      sx={{
                        fontSize: 12,
