...
def get_test_and_prompt(db: Session, test_id: str, organization_id: Optional[str] = None) -> Tuple[Test, str, str]:
    """
...
def check_existing_result(db: Session, test_config_id: str, test_run_id: str, test_id: str) -> Optional[Dict[str, Any]]:
...
def create_test_result_record(
    ...
    try:
        ...
        logger.debug(f"Successfully created test result with ID: {result.id if hasattr(result, 'id') else 'UNKNOWN'}")
    except Exception as e:
