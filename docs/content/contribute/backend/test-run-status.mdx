...
After all tests complete, the `collect_results` task runs (scheduled when the async batch or sequential runner finishes). `TestRunProcessor` in `result_processor.py` aggregates stored test results and sets the final run status:
...
- [Test Result Stats](/contribute/backend/test-result-stats) - Statistics APIs
