...
Get Rhesis running on your local machine in under 5 minutes without setup.

## Overview

This setup is intended for quickly testing Rhesis locally without configuration steps. It includes:

- ✅ **No external auth setup required** - Local authentication bypass enabled
...
- Local authentication bypass flag:
  - `QUICK_START=true` (enables backend auto-login; the frontend reads Quick Start status from the backend at runtime)
...
The local setup works out-of-the-box. To enable test generation, you need an AI provider. You have two options:

<Callout type="info">
By default, the system uses `rhesis/rhesis-default` as the AI model, which requires a `RHESIS_API_KEY`. If you prefer to use your own provider (e.g., OpenAI, Gemini), you must also set `DEFAULT_GENERATION_MODEL`, `DEFAULT_EVALUATION_MODEL`, and `DEFAULT_EXECUTION_MODEL` to that provider — otherwise the system will still try to use the Rhesis model and fail.
</Callout>
...
**Note:** The `./rh start` command automatically sets `QUICK_START=true` on the backend. The frontend discovers Quick Start mode from `GET /auth/providers` (`quick_start`). If you change `QUICK_START` after containers already exist, restart the stack: `./rh restart`.
