...
Two approaches:  (`./rh dev …`) for day-to-day work, or **all-in-one Docker** (`./rh start`) for a full environment without local service wiring.
...
**Tip:** run `./rh dev tmux` to start all of the above services in a single tmux session with one window per service, instead of running each command in its own terminal.
