...
# Resolves against the 'default' label
...
1. **By Label (Live config):**
   ```python
   # Evaluates the pointer and returns whatever it currently targets
   params = Parameters.get(project="support", label="production")
   ```
...
2. **Environment Variables:** `RHESIS_PARAMETERS_PIN` or `RHESIS_PARAMETERS_LABEL`. Lets ops pin a deploy without code changes.
3. **Explicit Kwargs:** `version` > `experiment_id` > `label` > implicit `label='default'`. Lets application code be explicit.
...
- **Label and Experiment ID entries** are cached with a Time-To-Live (TTL), defaulting to 60 seconds. Since the underlying version a label points to can change, stale entries trigger a refresh on the next access.
...
- `RHESIS_PARAMETERS_PIN=v_abc123` pins the application to an immutable version regardless of label movement. This acts as a production safety net or staged rollout mechanism.
- `RHESIS_PARAMETERS_LABEL=production` switches the application to a non-default label without requiring code changes.

This gives operators a code-free escape hatch to manage configuration rollouts.

## Error handling & graceful degradation

By default, `Parameters.get()` fails loudly. It raises `RhesisAPIError` on backend failure and `KeyError` on a missing label.
...
# 4. Move a label to point at the new version (the deploy step)
