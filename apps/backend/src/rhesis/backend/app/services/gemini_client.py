...
import logging
import re
from typing import AsyncGenerator, Generator, Iterator, List, Optional, Union, Dict, Any
...
def _get_model_name(client):
    ...
    return os.getenv("GEMINI_MODEL_NAME", DEFAULT_MODEL)
...
def _with_retries(func, *args, **kwargs):
    ...
    while retries <= MAX_RETRIES:
        try:
            ...
            is_api_error = (
                isinstance(e, errors.APIError) or
                "429" in str(e) or  # Rate limit
                "5" in str(e) and len(str(e)) >= 3 and str(e)[0] == "5" or  # 5xx error
...
"""
        ...
        if stream:
            ...
            def _error_stream():
                yield {
                    "choices": [
                        {"delta": {"content": f"I apologize, but I couldn't process your request due to an error: {str(e)}"}}
                    ]
                }
...
def create_chat_completion(request: dict):
    ...
    try:
        ...
        if stream:
            ...
            return {
                "choices": [
                    {"message": {"content": f"Error: {error_msg}"}}
                ]
            }
