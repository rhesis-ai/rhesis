...
If you have a product requirements document and want a **test foundation** on Rhesis before running tests, use the [PRD skill](/docs/skills/prd). Paste your PRD with `/rhesis-prd` to create behaviors, custom metrics, tags, and generated test sets via MCP — then use this platform skill to execute and analyze.
...
See also the [PRD skill](/docs/skills/prd) (`skills/rhesis-prd/`) for building a test foundation from requirements.
