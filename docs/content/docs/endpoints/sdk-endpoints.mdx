...
When your project uses **[Parameters & Experiments](/docs/experiments/parameter-schema)**, map resolved parameter values from `experiment_parameters` into your function arguments with `request_mapping`. This is the same `{{ experiment_parameters.* }}` path used by REST endpoints, so SDK and REST endpoints behave consistently during test runs.
...
@endpoint(
    ...
    request_mapping={
        ...
        "system_prompt": "{{ experiment_parameters.system_prompt | default('You are helpful.') }}",
    },
...
<Callout type="info">
  The older `@endpoint(parameters=["model", "temperature"])` kwarg-merging path
  and the `{{ params.* }}` template alias are both deprecated. They still work
  for existing applications, but new endpoints should use
  `{{ experiment_parameters.* }}` in `request_mapping`.
</Callout>
