...
<Callout type="default">
  **Single-turn** endpoints process one request-response pair per invocation.
  ...
  [Multi-Turn Conversations](/docs/endpoints/multi-turn-conversations).
</Callout>
...
<Callout type="info">
  **Note**: The example above hardcodes a single user message in the `messages`
  ...
  [Multi-Turn Conversations](/docs/endpoints/multi-turn-conversations#stateless-endpoints-message-history)
  for details.
</Callout>
...
When tests include file attachments (images, PDFs, or audio — see
[Multi-modal Testing](/docs/multimodal-testing) for the full type and size
matrix), the `files` variable contains an array of file objects. Each provider
expects files in a different format, so Rhesis provides built-in filters to
handle the conversion:

| Filter | Provider | Description |
|---|---|---|
| `to_anthropic` | Anthropic Claude | Images as `type: "image"`, PDFs as `type: "document"`, with `base64` source |
| `to_openai` | OpenAI | Files as `type: "image_url"` with `data:` URI |
| `to_gemini` | Google Gemini | Files as `inline_data` with `mime_type` and `data` |

Apply a filter using the pipe syntax:

<CodeBlock filename="anthropic-files-request.json" language="json">
  {`{
...
<CodeBlock filename="openai-files-request.json" language="json">
  {`{
  "model": "gpt-4o",
  "messages": [
    {
...
<CodeBlock filename="gemini-files-request.json" language="json">
  {`{
  "contents": [
    {
      "role": "user",
...
If your endpoint uses a custom file format, you can pass `files` directly
using `\{\{ files \}\}` and handle the transformation on your side.

<Callout type="info">
  See [Mapping Examples](/docs/endpoints/mapping-examples#file-attachments)
  for complete request and response configurations with file attachments.
</Callout>
...
- Conversation tracking fields (`conversation_id`, `session_id`, etc.): Used for
  [multi-turn conversation](/docs/endpoints/multi-turn-conversations) management

**Stored and available to metrics**:
- `metadata`: Stored with the test result and available to [custom metrics](/docs/metrics#using-metadata-in-evaluation) during evaluation
- `tool_calls`: Stored with the test result and available to metrics that evaluate tool use (e.g. tool correctness, function call accuracy)
...
[Endpoints Overview](/docs/endpoints#creating-an-endpoint) for details.
...
metrics. Usage metadata is stored alongside the test result for reference.
...
<Callout type="default">
  **Next Steps**
  ...
  - Return to the [Endpoints Overview](/docs/endpoints) for general
    endpoint management
  - Generate [Tests](/docs/tests-generation) to validate your endpoint
</Callout>
