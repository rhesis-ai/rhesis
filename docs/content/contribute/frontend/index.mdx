...
const apiClient = axios.create({
  // Browser: same-origin /api rewrite. Server: BACKEND_URL.
  baseURL: getBaseUrl(),
  timeout: 10000,
});
...
{`# Backend API proxy target for Next.js rewrites and server-side requests
...
<Callout type="default">
  **Next Steps** - Explore the [Component Library](/contribute/frontend/components)
  for available UI components - Check out [Authentication
  Guide](/contribute/frontend/authentication) for security implementation - Review [State
  Management](/contribute/frontend/state-management) for data flow patterns - Learn about [API
  Integration](/contribute/frontend/api-integration) for backend communication
</Callout>
