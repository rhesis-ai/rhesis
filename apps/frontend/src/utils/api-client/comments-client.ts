...
export class CommentsClient extends BaseApiClient {
  async getComments(entityType: string, entityId: string): Promise<Comment[]> {
    ...
    const response = await this.fetch<Comment[]>(`${API_ENDPOINTS.comments}/entity/${entityType}/${entityId}`);
    return response;
  }
  ...
  async updateComment(commentId: string, data: UpdateCommentRequest): Promise<Comment> {
    const response = await this.fetch<Comment>(`${API_ENDPOINTS.comments}/${commentId}`, {
      method: 'PUT',
      body: JSON.stringify(data)
    });
    return response;
  ...
  async addEmojiReaction(commentId: string, emoji: string): Promise<Comment> {
    const response = await this.fetch<Comment>(`${API_ENDPOINTS.comments}/${commentId}/emoji/${emoji}`, {
      method: 'POST'
    });
    return response;
  }

  async removeEmojiReaction(commentId: string, emoji: string): Promise<Comment> {
    const response = await this.fetch<Comment>(`${API_ENDPOINTS.comments}/${commentId}/emoji/${emoji}`, {
      method: 'DELETE'
    });
    return response;
