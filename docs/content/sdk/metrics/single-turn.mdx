...
<Callout type="info">
  **Do Not Answer detection**: Garak's `ToxicCommentModel` (`garak.detectors.unsafe_content`)
  is a local HuggingFace model that requires PyTorch. Rhesis uses `PerspectiveToxicity`
  (Google Perspective API) instead for do-not-answer and toxicity detection, avoiding the
  PyTorch dependency while providing equivalent coverage.
</Callout>
