...
def get_test_statistics(test_run: TestRun) -> Tuple[int, int, int]:
    ...
    tests_passed = sum(1 for result in test_results if result.status and result.status.name == 'passed')
    tests_failed = sum(1 for result in test_results if result.status and result.status.name == 'failed')
...
def determine_overall_status(tests_passed: int, tests_failed: int, total_tests: int, logger_func) -> Tuple[str, str]:
    """
    ...
    """
    logger_func('debug', f"Status calculation: tests_passed={tests_passed}, tests_failed={tests_failed}, total_tests={total_tests}")
...
def calculate_execution_time(test_run: TestRun, completion_time: datetime, logger_func) -> Optional[str]:
    """
    ...
    if not (test_run.attributes and isinstance(test_run.attributes, dict)):
        logger_func('debug', f"No test run attributes available or not a dict: {test_run.attributes}")
        return None
    ...
    if total_time_ms:
        ...
        logger_func('debug', f"Formatted execution time from total_execution_time_ms: {execution_time}")
        return execution_time
    
    # Calculate from started_at and current completion time
    started_at = test_run.attributes.get('started_at')
    logger_func('debug', f"Found started_at: {started_at}, using completion_time: {completion_time.isoformat()}")
    ...
    try:
        ...
        logger_func('debug', f"Calculated execution time from started_at to now: {execution_time} ({duration_seconds} seconds)")
        return execution_time
    except Exception as e:
        logger_func('warning', f"Failed to calculate execution time from timestamps", error=str(e))
        return None


def update_test_run_status(db, test_run: TestRun, overall_status: str, completion_time: datetime, 
                          execution_time: Optional[str], logger_func) -> None:
    """
    ...
    logger_func('info', f"Current test run status: {current_status_name}, updating to: {overall_status}")
    ...
    if execution_time and updated_attributes.get('started_at'):
        try:
            ...
            start_time = datetime.fromisoformat(updated_attributes['started_at'].replace('Z', '+00:00'))
            ...
            logger_func('warning', f"Failed to calculate total_execution_time_ms for storage", error=str(e))
    ...
    updated_test_run = crud.update_test_run(db, test_run.id, crud.schemas.TestRunUpdate(**update_data))
    
    # Verify the update worked
    if updated_test_run and updated_test_run.status:
        logger_func('info', f"Test run status successfully updated to: {updated_test_run.status.name}")
    else:
...
def build_summary_data(test_run_id: str, email_status: str, total_tests: int, tests_passed: int, 
                      tests_failed: int, execution_time: Optional[str], test_set_name: str,
                      endpoint_name: str, endpoint_url: str, project_name: str, 
...
class TestRunProcessor:
    ...
    def process_test_run_results(self, db, test_run: TestRun, test_run_id: str, 
                                completion_time: datetime) -> Dict[str, Any]:
        """
        ...
        self.logger_func('debug', f"Test statistics: total={total_tests}, passed={tests_passed}, failed={tests_failed}")
        ...
        test_set_name, endpoint_name, endpoint_url, project_name = get_test_context(test_configuration)
        ...
        update_test_run_status(db, test_run, overall_status, completion_time, execution_time, self.logger_func)
        
        # Build and return summary data
        return build_summary_data(
            test_run_id, email_status, total_tests, tests_passed, tests_failed,
            execution_time, test_set_name, endpoint_name, endpoint_url, project_name,
            completion_time
        ) 