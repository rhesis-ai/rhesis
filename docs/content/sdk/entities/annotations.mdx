...
Annotations.for_metric("Answer Relevancy")  # by metric name, case-insensitive
...
test_result.annotate("fail", "Went off script here.", turn="Turn 2")

# Labelling a test, the way a person labels one in the Explorer
test.annotate("fail", "Not a fair question.")`}
</CodeBlock>

`"pass"` and `"fail"` are resolved to the organization's status rows for you, and cached. Metric tuning uses `"accepted"` and `"rejected"`.
