...
<ThemeAwareImage
  ...
  alt="Test Runs page — pass rate, status, executor, and annotation count per run"
/>

## Test Runs List

Use the list to compare status, pass rate, executor, and annotation activity
across runs. The **Annotations** column shows how many tests carry a human
verdict. Hover the value to see how many of those were corrected, meaning the
human verdict changed the automated result.

Open the filter drawer and set **Annotations** to **With** or **Without** to
include only runs that have annotations or only runs that do not.
...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-test-run-details.webp"
  darkSrc="/screenshots/rhesis-ai-test-run-details-dark.webp"
  alt="Test run summary — pass rate, tests executed, failures and cost, with a verdict grid of every test broken down by requirement and metric"
/>
...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-test-results-overview.webp"
  darkSrc="/screenshots/rhesis-ai-test-results-overview-dark.webp"
  alt="Overview tab of a test result — status, prompt, response, expected response, retrieved context, and result tags"
/>
...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-test-results-conversation.webp"
  darkSrc="/screenshots/rhesis-ai-test-results-conversation-dark.webp"
  alt="Conversation tab of a test result — each turn with its message, the agent reply, and a per-turn pass or fail status"
/>
...
A test run also records the version of the application it tested, taken from the endpoint's [Version Information](/docs/endpoints/management#version-information) or reported by the endpoint in its own response.
...
An annotation records a human verdict over the automated evaluation. Open a result, use its **Annotations** tab, and see [Annotations](/docs/annotations) for the targets available and what an override does.
...
<CodeBlock filename="annotation-mention-format.txt" language="text">
...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-test-run-compare.webp"
  darkSrc="/screenshots/rhesis-ai-test-run-compare-dark.webp"
  alt="Test run comparison — baseline and current run side by side with pass rate, cost, tokens and model, the cost and token differences, and a test-by-test comparison below"
/>
...
Each test result records the metrics it was evaluated against. Which metrics run is resolved from a priority hierarchy (execution-time → test set → requirement) configured when you start the run. See [Test Execution](/docs/test-runs/execution#test-run-metrics) for how to set metrics at each level.
...
<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-test-results-metrics.webp"
  darkSrc="/screenshots/rhesis-ai-test-results-metrics-dark.webp"
  alt="Metrics tab of a test result — overall performance, the requirement it belongs to, and a pass or fail verdict with a reason per metric"
/>
