...
<CodeBlock filename="terminal" language="bash">
{`pip install "rhesis-sdk[haystack]>=0.6.0"`}
</CodeBlock>

### Usage

Haystack emits native OpenTelemetry spans for pipeline and component runs. Rhesis enables Haystack tracing, turns on content capture, and translates those spans into the `ai.*` schema before export.
...
# 3. Run pipelines normally - spans are captured automatically
result = pipeline.run({"prompt": {"query": "What is RAG?"}})`}
</CodeBlock>

<Callout type="info">
  **Order matters.** `auto_instrument("haystack")` must run **after** `RhesisClient` is created. The integration wraps the Rhesis OTLP exporter on the active tracer provider; if no Rhesis provider is configured, `enable()` returns `False` and Haystack spans are not translated.
</Callout>

### What Gets Traced

- Pipeline runs as `ai.agent.invoke` spans
- LLM generators as `ai.llm.invoke` with token counts and prompt/completion events
- Retrievers as `ai.retrieval` spans
- Tool components as `ai.tool.invoke` spans
- Component input/output when Haystack content tracing is enabled (on by default in this integration)

See `examples/telemetry/haystack_example.py` for a runnable demo.
...
| Haystack | Auto-instrument (OTel span translation) | `haystack` | Supported |
