...
interface GarakImportDrawerProps {
  ...
  onClose: () => void;
  /**
   * Called once the Garak import/generation task(s) have been queued. Import
   * and generation both run as background tasks, so this fires on "started",
...
export default function GarakImportDrawer({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // auto-close here. The resulting test set(s) appear in the test sets
      // list once their tasks finish; the user closes this drawer manually
      // after seeing the confirmation below.
  ...
  // for the user to close it explicitly.
  ...
  const isGarakListMode = !isOwasp && !importing && !isImportComplete;
    ...
    >
      ...
      {isOwasp ? (
        ...
        <>
          ...
          {/* Import Result — both static import and dynamic generation are
              fire-and-forget background tasks (large probe sets can produce
              thousands of tests, so the request only queues the work and
              returns immediately). There's no meaningful in-between state to
              show while `importing` — the Save button's own spinner (via
            ...
            <Stack spacing={3} alignItems="center" sx={{ pt: 2 }}>
              ...
              <Stack spacing={0.5} alignItems="center">
                ...
                <Typography
                  ...
                  textAlign="center"
                >
                  Running in the background — the test set(s) will appear in
                  your test sets list once complete.
                </Typography>
              </Stack>
                      ...
                      <Stack spacing={1}>
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          <Typography variant="subtitle2" fontWeight="medium">
                            ...
                            {importResult.dynamicResults.length !== 1
                              ? 's'
                              : ''}
                          </Typography>
                        </Stack>
