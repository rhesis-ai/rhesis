...
**Location**: `jobs/execution/executors/results.py` → `link_traces_for_test_result()`
...
**Location**: `jobs/telemetry/post_ingest.py` → `link_traces_for_incoming_batch()`, run inside the async `post_ingest_link` task dispatched by the ingestion endpoint
