...
| **Requirements** | Yes | What the endpoint should (or shouldn't) do. Each test is tagged with a requirement. |
| **Test sets** | Yes | Named collections of tests, each targeting specific requirements, categories, and topics. |
| **Metrics** | Yes | How each test is evaluated — the evaluation criteria and pass/fail threshold. |
| **Requirement-metric mappings** | Yes | Which metric evaluates which requirement. Every requirement needs at least one. |

## Reuse status

Architect checks what already exists on your platform before proposing new entities. Each requirement and metric in the plan is labelled:
...
This prevents duplicate requirements and metrics from accumulating across sessions.

## Naming conventions

Architect uses **Title Case** for requirement and metric names — two to five words describing what is being measured or expected.
...
When planning, Architect shows each metric's scope and validates that every requirement in a test set has at least one linked metric with a compatible scope. If you need both one-shot guardrails and conversational retention tests, expect **two test sets** with separately scoped metrics.
...
- Add specific requirements Architect didn't include
