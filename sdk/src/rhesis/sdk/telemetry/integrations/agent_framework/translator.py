...
def conversation_root_attributes(span: ReadableSpan) -> dict[str, Any] | None:
    ...
    A run root that is *not* the trace root stamps nothing and queues the
    release without reading. That is safe even when a trace root will read
    afterwards, because the queue frees nothing on the way in and a read moves a
    trace back to the newest, so the root's own read refreshes the entry. When
    nothing will read it - a Rhesis ``@endpoint``/``@observe`` span owns the turn
    - it is what keeps the entries from sitting there until the eviction cap.
    ...
    if getattr(span, "parent", None) is not None:
        # Queue the release even though a trace root may still read this. The
        # queue frees nothing on the way in and a read moves a trace back to the
        # newest, so the root's own read refreshes the entry; and when no MAF
        # span roots the trace - an enclosing @endpoint/@observe span owns the
        # turn - nothing reads, and this is what stops the entries sitting there
        # until the eviction cap. Same choice the Google ADK translator makes.
    ...
    * It does **not** dedup ``@observe.llm`` against MAF.
    ...
    def on_start(self, span, parent_context=None) -> None:  # noqa: D401
        ...
        try:
            ...
            # Claim this turn's trace from the span that roots it, which starts
            # before every span whose trace id the exporter will rewrite. Only a
            # run root, so a bare chat span - a chat client called with no agent
            # around it - is left alone: nothing stamps a chat span, so claiming
            # would join those turns onto one trace carrying no conversation id
            # anywhere. Restricting it to the root also keeps a conversation id
            # changed mid-run from quietly merging two conversations.
            #
            ...
            maf_span_name = getattr(span, "name", None)
            roots_the_trace = getattr(span, "parent", None) is None and (
                mapping.is_workflow_run_span(maf_span_name)
                or mapping.is_agent_invocation_span(maf_span_name)
            )
            if (
                roots_the_trace
                and get_root_trace_id() is None
                and get_conversation_trace_id() is None
            ):
