...
The Test Run detail page displays an **Experiment** card showing the experiment name, short version hash (e.g., `v_a3f9b8`), the source label (if any), and an expandable **Resolved Values** panel. (Values for `secret_ref` slots are masked so credentials never leak into the test run record.)
...
## Multi-turn Result Details (v0.6.7+)
...
## Cancel In-Progress Test Runs (v0.6.12+)
...
### Review targets and override behavior (v0.6.9+)
