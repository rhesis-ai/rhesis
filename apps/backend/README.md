...
For local development, bring up Postgres and Redis from the repository root (`./rh dev init` once, then `./rh dev up`) and ensure `apps/backend/.env` points at that instance. Apply the schema from `apps/backend` with:
...
From the repository root, run the image with `apps/backend/.env.docker` (or your own env file) and port `8080` as required.
