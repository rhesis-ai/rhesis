...
Metrics are organized by [**Behaviors**](/platform/behaviors), which are atomic expectations you have for the output of your application.
...
To learn how to enable metrics for live production traffic, read the [**Trace Metrics Guide**](/platform/metrics/trace-metrics).
...
Select the [Model](https://app.rhesis.ai/models) you want to use for the evaluation. For more information on how to configure models, see the [Models documentation](/platform/models).
</details>
...
To use these fields, the metric only needs to make this explicit in its configuration (e.g., in the **Evaluation Prompt**). The evaluation model will automatically receive these objects if they are present in the interaction and reason about them when producing a score.
...
- **Trace**: Live production traffic evaluated in real-time (see [Trace Metrics](/platform/metrics/trace-metrics))
