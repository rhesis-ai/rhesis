...
class TemplateService:
    ...
    def render_template(self, template: EmailTemplate, variables: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(f"Successfully rendered template {template.value} with {len(context)} variables")
    ...
    def _prepare_context(self, template: EmailTemplate, variables: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if missing_vars:
            logger.debug(f"Template {template.value} missing variables (set to N/A): {missing_vars}")
