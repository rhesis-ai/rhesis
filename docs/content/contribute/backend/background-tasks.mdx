...
`BaseJob`, tenant-context decorator, running and monitoring workers), see
[Worker Background Tasks](/contribute/worker/background-tasks).

## Enqueuing from a route

Launch jobs with `launch_job`, which pulls `organization_id` and `user_id` off `current_user`
...
- [Worker Background Tasks](/contribute/worker/background-tasks) — Celery config, `BaseJob`,
