...
Test sets are collections of individual test entities used to evaluate LLM applications. Each test entity contains a prompt, expected requirements, and metadata for categorization.
...
| `requirements` | `list[str]` | Requirement names in this set |
...
tests = [
    ...
    Test(
        category="Safety",
        topic="Privacy",
        requirement="Protects user data",
...
`test_set.get_annotations()` returns every human verdict left on the results and traces its runs produced. See [Annotations](/sdk/entities/annotations).
...
| Dict with `id` | `test_set.add_tests([{"id": "uuid"}])` |
| UUID string | `test_set.add_tests(["uuid"])` |

<CodeBlock filename="associate_tests.py" language="python">
...
tests = [
    Test(category="Safety", topic="Weapons", requirement="Refuses", prompt=Prompt(content="...")),
    Test(category="Safety", topic="Violence", requirement="Refuses", prompt=Prompt(content="...")),
]
...
| `requirement` | `str` | Expected requirement (e.g., "Refuses harmful requests") |
...
A `Prompt` is the input side of a single-turn test. It is a value on a `Test` rather than something you fetch or manage on its own.

| Property | Type | Description |
|----------|------|-------------|
| `content` | `str` | What is sent to the endpoint. The one field the API requires |
| `expected_response` | `str \| None` | What a correct answer looks like. Omit it unless a metric compares the response against a reference |
| `language_code` | `str` | Defaults to `en` |

Multi-turn tests carry a `test_configuration` instead. Set the one that matches the test: every route requires at least one, and sending both is accepted when creating a test but rejected when executing an ad-hoc one whose `test_configuration` carries a `goal`.
...
test = Test(
    ...
    requirement="Provides correct information",
    prompt=Prompt(
...
test = Test(
    ...
    requirement="Maintains context across turns",
