...
Without authentication, the service allows 100 requests per day per IP address; with authentication, 1000 per day per user (configurable via `CHATBOT_RATE_LIMIT`). Limits use a rolling 24-hour window; exceeding them returns `429 Too Many Requests`.
