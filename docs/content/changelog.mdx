...
**Operator runbook:** See `apps/backend/scripts/migrate_files_to_storage.py` and the Phase 8b deployment runbook in `playground/async-migration/ASYNC_MIGRATION_PLAN.md`. The deprecated `content` column is retained until runbook step 7 (after soak verification).
