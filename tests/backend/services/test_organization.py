...
class TestLoadInitialData:
    ...
    def test_load_initial_data_success(self, test_db: Session, authenticated_user_id, test_org_id):
        ...
        initial_type_lookup_count = (
            test_db.query(models.TypeLookup)
            .filter(models.TypeLookup.organization_id == test_org_id)
            .count()
        )
        ...
        assert final_type_lookup_count >= initial_type_lookup_count, (
            "TypeLookup entities should exist (may already be present)"
        )
        ...
        test_status = (
            test_db.query(models.Status)
            .filter(
                models.Status.organization_id == test_org_id, models.Status.name.ilike("%test%")
            )
            .first()
        )
    ...
    def test_load_initial_data_with_custom_file_path(
        ...
        try:
            ...
            original_path = organization_service.__file__.replace(
                "__init__.py", "initial_data.json"
            )
            ...
            assert "test status" in test_status.description.lower(), (
                f"Expected description to contain 'test status', got: {test_status.description}"
            )
    ...
    def test_load_initial_data_invalid_json(
        self, test_db: Session, authenticated_user_id, test_org_id
    ):
    ...
    def test_load_initial_data_integration(
        ...
        project_root = os.path.dirname(
            os.path.dirname(os.path.dirname(test_file_dir))
        ...
        if expected_initial_data.get("status"):
            ...
            assert created_status is not None, (
                f"Status '{first_status['name']}' should exist for this organization"
            )
        ...
        if expected_initial_data.get("topic"):
            ...
            assert created_topic is not None, (
                f"Topic '{first_topic['name']}' should exist for this organization"
            )
        ...
        assert entity_type_lookup is not None, (
            "EntityType lookup should exist for this organization"
        )
...
class TestRollbackInitialData:
    ...
    def test_rollback_initial_data_success(
        self, test_db: Session, authenticated_user_id, test_org_id
    ):
        ...
        prompts_before = (
            test_db.query(models.Prompt)
            .filter(models.Prompt.organization_id == test_org_id)
            .count()
        )
        assert prompts_before > 0

        # Call the rollback function
        organization_service.rollback_initial_data(db=test_db, organization_id=test_org_id)
    ...
    def test_rollback_initial_data_organization_not_found(self, test_db: Session):
        ...
        with pytest.raises(ValueError, match="Organization not found"):
            organization_service.rollback_initial_data(db=test_db, organization_id=non_existent_id)

    def test_rollback_initial_data_organization_not_initialized(
