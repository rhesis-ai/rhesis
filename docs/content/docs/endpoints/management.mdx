...
The route validates that the endpoint belongs to the current tenant, starts a Celery task, and returns a `task_id`. Poll `GET /jobs/{task_id}` until the job reaches `SUCCESS`; the job result contains the exploration findings, conversation summary, strategy, duration, and endpoint ID.
