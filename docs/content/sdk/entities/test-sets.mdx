...
### Adaptive TestTree node metadata (v0.6.12+)

If you use adaptive testing with the SDK `TestTree` APIs, each `TestTreeNode` can carry
evaluation metadata in addition to `label` and `model_score`.

This is useful when you want to inspect per-metric outcomes programmatically after adaptive
evaluation runs.
