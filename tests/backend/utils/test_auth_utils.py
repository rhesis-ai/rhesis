...
class TestGetCurrentUser:
    ...
    def test_get_current_user_with_session_user_found(self):
        ...
        with patch('rhesis.backend.app.database.get_db', mock_get_db):
            with patch('rhesis.backend.app.auth.auth_utils.get_user_by_id', return_value=mock_user) as mock_get_user:
                result = pytest.run(get_current_user(request))
...
class TestVerifyJwtToken:
    ...
    def test_verify_jwt_token_success(self):
        ...
        with patch('rhesis.backend.app.auth.auth_utils.jwt.decode', return_value=expected_payload) as mock_decode:
    ...
    def test_verify_jwt_token_invalid(self):
        ...
        with patch('rhesis.backend.app.auth.auth_utils.jwt.decode', side_effect=JWTError("Invalid token")):
...
class TestValidateToken:
    ...
    def test_validate_token_success_with_usage_update(self):
        ...
        with patch('rhesis.backend.app.auth.auth_utils.get_token_by_value', return_value=mock_token) as mock_get_token:
    ...
    def test_validate_token_success_without_usage_update(self):
        ...
        with patch('rhesis.backend.app.auth.auth_utils.get_token_by_value', return_value=mock_token):
...
class TestGetAuthenticatedUserWithContext:
    ...
    def test_get_authenticated_user_session_only_success(self):
        ...
        with patch('rhesis.backend.app.auth.auth_utils.get_current_user', return_value=mock_user) as mock_get_current:
    ...
    def test_get_authenticated_user_jwt_success(self):
        ...
        with patch('rhesis.backend.app.auth.auth_utils.get_user_from_jwt', return_value=mock_user) as mock_get_jwt_user:
...
pytest.run = lambda coro: pytest.run_async_helper(coro) if hasattr(pytest, 'run_async_helper') else None
