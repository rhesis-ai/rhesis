...
When execution finishes, the `collect_results` task runs `TestRunProcessor` (`jobs/execution/result_processor.py`), which aggregates stored test results and calls `determine_overall_status`:
...
| File | Purpose |
| ---- | ------- |
| `jobs/execution/result_processor.py` | Status determination and statistics |
| `jobs/enums.py` | `RunStatus` enum |
