...
<Callout type="info">
  **Do Not Answer detection**: Garak's `ToxicCommentModel` is a local model that cannot be
  run ad hoc. When importing test sets via the Rhesis platform, `PerspectiveToxicity`
  (Google Perspective API) is used instead for do-not-answer and toxicity detection.
</Callout>
