...
- A name the geocoder cannot place is recorded in `resolution_attempts` and not retried — but the *reason* decides, not the presence of an attempt. `UNPLACEABLE_REASONS` covers the verdicts that mean the name is the problem (`no match`, `country, not a city`, an empty answer); a `timeout` or `error` says nothing about the name, so it neither marks the destination unplannable nor counts as already tried. Conflating the two stalled the conversation outright: a transient Nominatim outage drove `missing_slots()` to `['city']`, so the coordinator asked which city with the city already on file, and answering recorded the same name and asked again.
