...
### Changed
- **Configuration**: Streamlined API configuration by replacing `RHESIS_BASE_URL` with `API_BASE_URL` for OAuth callbacks. `API_BASE_URL` is now a required setting in `ApplicationSettings` with no default value, improving compatibility and setup clarity for hybrid self-hosted deployments.

### Fixed
- **Asynchronous Tasks**: Fixed a database scoping issue in the `enrich_trace_async` Celery task by properly binding the tenant scope to the session, preventing `''::uuid` casting errors when project-wide Row-Level Security (RLS) is enabled.
- **REST Invoker**: Resolved "Event loop is closed" errors during sequential test execution by ensuring the HTTP client is recreated when the active event loop changes.
- **Changelog Generation**: Upgraded internal release tooling to use `gemini-3.5-flash`, increased maximum token limits, and resolved fallback truncation and formatting issues.


## [0.9.0] - 2026-06-11

### Added
-   **Endpoint Creation Wizard:** Introduced a new interactive wizard and test-and-map UI for streamlined endpoint creation and configuration.
-   **Project Isolation & Security:** Implemented comprehensive project-level isolation, ambient scope, and security hardening across the backend, including core support for project-container scoping.
-   **Behavior Tags:** Added functionality to tag behaviors, allowing for better organization and grouping.
-   **Redis Chatbot Sessions:** Integrated Redis for robust and scalable chatbot session management.
-   **Structured Production Logging:** Enhanced backend observability with structured logging for production environments.

### Changed
-   **Configuration Refactoring:** Centralized configuration management: unified environment detection under `BACKEND_ENV` and separated application from administrative database credentials.
-   **Connector Architecture:** Improved internal architecture for connectors and endpoints, removing local backend execution in favor of `EndpointContext`.
-   **Test Environment:** Enhanced test environment configuration to explicitly allow `BACKEND_ENV=test` in settings.

### Fixed
-   **Test Execution Reliability:** Resolved 'Event loop is closed' errors, improving the reliability of sequential test execution.
-   **Data Isolation:** Corrected tenant scope binding in Celery tasks (`enrich_trace_async`) to ensure proper data isolation.
-   **Database Migrations:** Eliminated the superuser requirement for Row-Level Security (RLS) migrations, simplifying database management.
-   **Metric Evaluation:** Addressed metric evaluation failures for single-turn tests.
-   **Authentication & Onboarding:** Improved Invoker token management and fixed 403 errors during onboarding caused by authentication backstops.
-   **Architect Chat RLS:** Ensured correct Row-Level Security (RLS) for architect chat messages, enhancing data security.
-   **Endpoint Response Mapping:** Corrected endpoint response mapping and guarded against non-dictionary SDK outputs.
-   **Architect Agent Observability:** Improved observability for the Architect agent with coherent multi-turn traces and conversation views.
-   **Multi-turn Conversations:** Fixed system prompt stripping and improved SDK usage for multi-turn conversations.
-   **Developer Experience:** Allowed localhost OAuth redirects in development environments for smoother developer workflows.
-   **Preflight Checks:** Enhanced preflight check performance and reliability.

### Removed
-   **Deprecated Configuration:** Removed back-compatibility with old environment variable names (users may need to update their configurations).
-   **Internal Package:** Streamlined the codebase by removing the internal `services/mcp` package.
...
### Added
- Introduced Rhesis Architect, an AI agent designed to enhance test suite design and execution capabilities.
- Implemented flexible model selection and execution support, allowing for broader integration of diverse models.
- Added adaptive testing embeddings and diversity-aware suggestions to improve the relevance and quality of test generation.
- Enabled setting default model IDs in user settings during local initialization.
- Enhanced telemetry reliability by adding retry mechanisms with exponential backoff to data exporters.

### Changed
- Temporarily disabled the default embedding model selection feature.

### Fixed
- Resolved issues with model connection endpoint passthrough.
- Corrected synchronization of topic relationships following foreign key updates.
- Improved pipeline reliability and addressed issues with Garak probe-coupled detectors.
- Fixed an issue preventing email notifications from being sent upon test generation completion.
- Updated the exchange rate API URL and enhanced fallback caching logic for improved data retrieval.
- Addressed UUID conversion errors.

### Security
- Patched several vulnerable dependencies identified by security audits.
