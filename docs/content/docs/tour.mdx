...
carry a requirement, topic, and category, and group into [test sets](/docs/test-sets) for batch runs.

<ThemeAwareImage
  lightSrc="/screenshots/rhesis-ai-tests.webp"
  darkSrc="/screenshots/rhesis-ai-tests-dark.webp"
  alt="Tests page — saved cases with their requirement, topic, category, and type"
/>
...
A saved conversation says what happened. [Requirements](/docs/requirements) and
[metrics](/docs/metrics) say what should happen: a requirement is the expectation, and the metrics
attached to it are the judges that score responses against it. Results roll up by requirement, so you
...
<ThemeAwareImage
  ...
  alt="Metrics page — each metric card shows the requirements it measures and its backend"
/>
...
[annotation](/docs/annotations) is a human verdict that overrides the automated one, at the
...
<ThemeAwareImage
  ...
  alt="Annotating a test run — human Pass or Fail verdicts with comments alongside the automated scores"
/>
...
results, metric scores, conversation history, and annotation activity, so "is it getting better" becomes
...
<ThemeAwareImage
  ...
  alt="Test Runs page — pass rate, status, executor, and annotation count for each run"
/>
...
Annotations are attached to what they judge, so a script can pick up what the team decided:

<CodeBlock filename="read_annotations.py" language="python">
{`from rhesis.sdk.entities import Annotations, TestRuns

run = TestRuns.all()[0]

# Everything annotated anywhere in the run: its results and its traces
for annotation in Annotations.for_test_run(run.id):
    target = annotation.target_reference or annotation.target_type
    print(target, annotation.status.name, annotation.user.name)
