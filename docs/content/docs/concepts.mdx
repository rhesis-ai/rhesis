...
Test sets can optionally override which metrics run; otherwise metrics come from each test's behavior. When a run uses an [experiment](/docs/experiments), the resolved configuration is snapshotted on the test run.

---

<Callout type="default">
  **Ready to start?** Create a [project](/docs/projects), [connect an endpoint](/docs/getting-started/connecting-application), and [generate tests](/docs/tests-generation) to run your first evaluation.
</Callout>
