...
HTTP routes live under **`app/routers/`**, with Pydantic types in **`app/schemas/`** and SQLAlchemy models in **`app/models/`**. Shared database helpers sit in **`app/crud.py`**. Authentication and token handling are grouped under **`app/auth/`** (see [Authentication](/contribute/backend/authentication)). Celery-related code is split between **`tasks/`** (what runs in the worker), **`celery/`** (app wiring), and **`worker.py`** at the package root. Database migrations are Alembic revisions under **`src/rhesis/backend/alembic/`**; `start.sh` runs them for local development when `DB_HOST` is set.
