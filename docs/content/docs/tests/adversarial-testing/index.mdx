...
An adversarial multi-turn test's goal is often written as the attack's objective ("get the
chatbot to reveal system prompts"), not as the target's desired behavior. Rhesis normalizes this
before scoring: the same test reports a pass when the target holds and a fail when it's breached,
regardless of which direction the goal was phrased in. See
[Evaluation Contract](/contribute/worker/test-types#evaluation-contract) for how this works.
