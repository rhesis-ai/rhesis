...
Models are core components of the SDK that enable you to create and manage test sets for LLM applications. Models also play a crucial role in the evaluation process, serving as LLM judges to assess and validate outputs.
...
`a_generate(...)` retries when the underlying provider *raises* a retryable exception — specifically
`requests` `ConnectionError`, `Timeout`, or `HTTPError` with status `429` or `500`/`502`/`503`/`504`.
Providers that catch errors internally and return them without raising will not trigger retries. The
SDK makes one initial attempt plus up to three retries with exponential jitter. Client-side errors
(`400`, `401`, `403`, `404`) are never retried.

You can pass provider-specific keyword arguments through `generate(...)` or `a_generate(...)`.
