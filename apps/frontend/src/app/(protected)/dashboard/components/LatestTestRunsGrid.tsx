...
import { Typography, Box, CircularProgress, Alert, Tooltip } from '@mui/material';
...
const STATUS_MAP: Record<string, { label: string, color: string }> = {
  'da08d71e-d23e-4956-9326-53b184859287': { label: 'Failed', color: 'error.main' },
  '868a686f-349e-4527-a8dd-f1a3b987aa7b': { label: 'Completed', color: 'success.main' },
...
export default function LatestTestRunsGrid({ sessionToken }: LatestTestRunsGridProps) {
  ...
  const testRunsColumns: GridColDef[] = [
    { 
      ...
      renderCell: (params) => {
        ...
        } else if (row.attributes?.task_state && TASK_STATE_COLORS[row.attributes.task_state]) {
    ...
    { 
      ...
      valueGetter: (_, row) => {
        ...
        try {
          return format(parseISO(row.attributes.started_at), 'yyyy-MM-dd HH:mm:ss');
        } catch (e) {
          return '';
        }
      }
    ...
    { 
      ...
      valueGetter: (_, row) => {
        if (row.user) {
          return `${row.user.given_name || ''} ${row.user.family_name || ''}`.trim() || row.user.email || row.user_id;
        }
        return row.user_id ? `User ID: ${row.user_id.toString().substring(0, 8)}...` : 'Unknown';
      },
      renderCell: (params) => (
        <Box sx={{ width: '100%', display: 'flex', alignItems: 'center' }}>
