...
export default function TestSetDetailCharts({ testSetId, sessionToken }: TestSetDetailChartsProps) {
  ...
  const generateDimensionData = useCallback((dimension: string): ChartDataItem[] => {
  ...
  useEffect(() => {
    const fetchTestSetStats = async () => {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'Failed to load test statistics for this test set';
