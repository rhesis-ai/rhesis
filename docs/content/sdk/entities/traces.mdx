...
`status_code` on a listing is the **root** span's. A trace whose inner LLM call failed can still show `OK`, which is what `root_spans_only=False` is for: it matches the spans themselves.
