...
An AI provider is required for test generation and evaluation. By default, the system uses `rhesis/rhesis-default` as the model, which requires a `RHESIS_API_KEY`. If you do not configure either a `RHESIS_API_KEY` or an alternative provider with matching `DEFAULT_GENERATION_MODEL` and `DEFAULT_EVALUATION_MODEL`, test generation will fail.
