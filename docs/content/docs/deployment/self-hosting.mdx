...
An AI provider is required for test generation, evaluation, and multi-turn execution. By default, the system uses `rhesis/rhesis-default` as the model, which requires a `RHESIS_API_KEY`. If you do not configure either a `RHESIS_API_KEY` or an alternative provider with matching `DEFAULT_GENERATION_MODEL`, `DEFAULT_EVALUATION_MODEL`, and `DEFAULT_EXECUTION_MODEL`, test generation and execution will fail.

**Option 1: Use Rhesis API (Recommended)**

Set your Rhesis API key — no other model configuration is needed since the defaults already use `rhesis/rhesis-default`:
...
Set your provider's API key **and** change the default models to use that provider:
...
Supported provider prefixes: `openai`, `gemini`, `azure`, `anthropic`, `groq`, `mistral`, `together_ai`, `perplexity`, `replicate`, `openrouter`, `cohere`, `ollama`.
