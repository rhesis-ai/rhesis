...
Spans include attributes for model name, provider, token counts (input/output), and tool names. To get `ai.agent.invoke` spans, node names must contain an agent keyword — see [Multi-Agent Tracing](/tracing/multi-agent).
