...
If the service is configured with an API key, chat and session requests require a bearer token in the `Authorization: Bearer <your-api-key>` header. `/health`, `/`, and `/use-cases` are always accessible without authentication.
...
Without authentication, the service allows 100 requests per day per IP address; with authentication, 1000 per day per user. Limits reset at midnight UTC. Responses carry `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset` headers; exceeding the limit returns `429 Too Many Requests` with a `Retry-After` header.
...
- **No Persistence**: Session data may be cleared during deployments
...
<Callout type="default">
  **Next Steps** - Create [custom endpoints](/docs/endpoints) for your own AI
  services - Explore [test generation](/docs/explorer/tests-generation) to validate your
  AI applications - Set up [metrics](/docs/metrics) to measure response quality
</Callout>
