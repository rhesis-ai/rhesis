...
<Callout type="info">
  **This page has moved!** Test sets are now available under [Test Sets](/docs/test-sets).
</Callout>
...
#### Executing and Rescoring via SDK

The SDK supports per-run model overrides when executing test sets or rescoring existing runs:

<CodeBlock
  filename="execute_test_set.py"
  language="python"
>
{`from rhesis.entities import TestSet, Endpoint

test_set = TestSet(id="test-set-id")
endpoint = Endpoint(id="endpoint-id")

# Execute with model overrides
result = test_set.execute(
    endpoint,
    execution_model_id="<model-uuid>",   # Override execution model (multi-turn)
    evaluation_model_id="<model-uuid>",   # Override evaluation model
)

# Re-score an existing run with a different evaluation model
result = test_set.rescore(
    endpoint,
    evaluation_model_id="<model-uuid>",
)`}

</CodeBlock>

You can also set the default execution model for your account:

<CodeBlock
  filename="set_default_execution.py"
  language="python"
>
{`from rhesis.entities import Model

model = Model(id="model-uuid")
...
<Callout type="default">
  **Next Steps** - Execute your test set against an
  [Endpoint](/docs/endpoints) - View detailed results in [Test
  Runs](/docs/test-runs) - Download test sets as CSV or access via
