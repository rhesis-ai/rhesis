...
const API_BASE_URL = getBaseUrl(); // browser: same-origin /api; server: BACKEND_URL
...
The backend URL for rewrites and server-side calls is `BACKEND_URL`. The browser calls same-origin `/api/*`, which Next.js proxies to that backend.
