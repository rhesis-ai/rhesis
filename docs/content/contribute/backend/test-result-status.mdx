...
`determine_status_from_metrics` (`jobs/execution/executors/metrics.py`) and `create_test_result_record` (`jobs/execution/executors/results.py`) apply the same rule:
...
Read `test_metrics` directly only for per-metric analytics, drill-down, and debugging — never to recompute overall pass/fail. Note that human [annotations](/contribute/backend/annotations) can override the stored status or individual metric outcomes; the stats views backing Insights account for those overrides.
...
- [Annotations](/contribute/backend/annotations)
