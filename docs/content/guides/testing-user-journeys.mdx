...
- Name **user journey-specific requirements** (concrete expectations) and attach metrics to them
- Turn user journey flows into sets of **multi-turn tests**
- Execute test sets against an endpoint and interpret results

Summarized, this guide shows how to test one flow slice of your user journey systematically in Rhesis, and how Requirements, Metrics, and Tests fit together.
...
| `Emotions / UX quality` | Clarity, trust, frustration, confidence | Journey-specific `Requirement` + `Metrics`; optional `Restrictions` defining what the system must not do |
...
- **Requirement**: what “good” looks like. For user journeys, name requirements after the journey and one concrete expectation that you want to assess.
- **Metric**: how we measure that expectation (LLM-as-judge evaluation that returns pass/fail, often with a score and reasoning).
- **Test**: a case to be tested. In Rhesis, each test is tagged with **one requirement** (plus optional topic/category metadata).
...
|---|---|---|
| **Requirement** | The expectation you care about | `Flight booking - completion` where “complete” means the booking reaches an issued ticket / confirmed itinerary state |
...
### Step 2: Create 1-3 requirements for this flow
...
### Step 3: Attach custom metrics to requirements

Attach custom metrics to these requirements in the platform.
...
### Step 4: Create 2-3 multi-turn tests per requirement

Create 2-3 tests per requirement, each targeting a different persona or scenario variant (e.g., a business traveler with tight schedule constraints vs. a leisure traveler with flexible dates). The goal is to cover the most common and representative user contexts without over-testing a single flow slice. You can add more variants later when you scale to a full journey suite.

Create **Multi-Turn** tests with:

- **Requirement**: pick the expectation you want to evaluate (for example `Flight booking - completion`).
...
| Field | Value |
|-------|-------|
| **Requirement** | Flight booking - completion |
...
- **Why did it fail?** Read the metric reasoning to understand what went wrong. Check whether the issue is in the requirement definition (expectation too strict or too loose), the metric prompt (evaluation criteria unclear), or the endpoint itself (the AI system genuinely underperformed).

Iterate by adjusting the requirement, metric, or test scenario and re-running. See [Test Runs](/docs/test-runs) for how to compare runs and track improvements over time.
