...
- **Celery tasks** — add the job under `jobs/` and register it in `jobs/__init__.py`. Run a worker with `./rh dev worker` to exercise it (see [Background tasks](/contribute/backend/background-tasks)).
