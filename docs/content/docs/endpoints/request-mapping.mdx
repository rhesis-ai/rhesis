...
When tests include file attachments, the `files` variable contains an array of objects with `filename`, `content_type`, and `data` (base64-encoded). See [Multi-modal Testing](/docs/multimodal-testing) for the full type and size matrix.
...
<Callout type="default">
  **Next Steps**
  - See [Response Mapping](/docs/endpoints/response-mapping) to extract values from API responses
  - Browse [Examples](/docs/endpoints/mapping-examples) for complete provider configurations
</Callout>
