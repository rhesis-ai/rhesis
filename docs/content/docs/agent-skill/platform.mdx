...
The **Rhesis agent skill** (`rhesis`) packages platform knowledge into one portable skill file: explore endpoints, design test suites, generate and run tests, and analyze results. For **building a test foundation from requirements first**, see the [requirements workflow](/docs/agent-skill/requirements) — same install, same skill.
...
If you have a Product Requirements Document (PRD) or product spec and want a **test foundation** before running tests, paste your requirements in chat — the skill routes to the [requirements workflow](/docs/agent-skill/requirements). After approval it creates behaviors, metrics, tags, and test sets via MCP; then continue here to execute and analyze.
...
See also the [requirements workflow](/docs/agent-skill/requirements) (`skills/rhesis/references/prd/`) for building a test foundation from requirements.
