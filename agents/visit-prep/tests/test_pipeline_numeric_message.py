...
def test_numeric_message_stays_string_through_pipeline():
    ...
    result = run_turn(
        "9",
        VisitPrepState(chief_complaint="nausea"),
        pipeline=pipeline,
...
def test_router_preserves_messages_with_special_chars(message: str):
    ...
    native evaluation.
    """
    from visit_prep.pipeline import _build_intent_conditional_router

    router = _build_intent_conditional_router()
    out = router.run(intent="health_concern", message=message, state=VisitPrepState())
...
def test_special_char_message_completes_turn(message: str):
    ...
    pipeline = build_intent_pipeline(components)
    result = run_turn(message, VisitPrepState(), pipeline=pipeline, components=components)
