...
<FileTree
  ...
  data={[
    ...
    { name: 'external-secrets.yaml', type: 'file', description: 'Application: ESO Helm chart' },
...
<Callout type="info">
This uses an HTTP-01 challenge, which needs no DNS provider API — it requires your domain's DNS
to point at the ingress-nginx `LoadBalancer` IP (created below) before certificates can issue. Rhesis's own environments use Cloudflare + DNS-01 instead, which isn't required here.
</Callout>
...
- **Add DNS automation or an internal-only ingress** — for managed DNS records or a VPN-only
  surface alongside the public one
