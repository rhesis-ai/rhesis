 ...
 */
export function createReactionTooltipText(reactions: Array<{ user_name?: string }>, emoji: string): string {
  ...
  if (reactionCount === 2) {
    const userNames = reactions.map(reaction => reaction.user_name || 'Unknown User');
  ...
  // 4+ reactions
  const firstThreeNames = reactions.slice(0, 3).map(reaction => reaction.user_name || 'Unknown User');
