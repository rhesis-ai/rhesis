...
# 3. (Optional) Add your Rhesis API key to .env.local
# Open .env.local and update RHESIS_API_KEY with your key from https://app.rhesis.ai/
...
<FileTree
  ...
  data={[
    {
      name: '.env.example.local',
      type: 'file',
      description: 'Template with all pre-configured defaults'
    },
    {
      name: '.env.local',
      type: 'file',
      description: 'Your local copy (created from .env.example.local)'
    },
    {
...
The `.env.example.local` file includes:
...
By default, the local setup uses the Rhesis API for test generation. You'll need to:
